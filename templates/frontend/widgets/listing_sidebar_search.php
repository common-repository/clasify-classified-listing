...
class Listing_Search_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $pagenow !== 'widgets.php' ) {
			$cl_search_fields_data   = CCP()->front->listing_provider->cl_search_fields_data();
			$listing_post_type       = CCP()->front->listing_provider->get_cpt_lists( true );
				...
				<?php if ( isset( $instance['title'] ) && ! empty( $instance['title'] ) ) { ?>
					<div class="row">
						<div class="col-lg-12 col-md-12 col-sm-12 mb-2">
							<h6><?php echo esc_html__( $instance['title'], 'clasify-classified-listing' ); ?></h6>
						</div>
					</div>
				<?php } ?>
				<form method="get" id="advanced-searchform" role="search" action="<?php echo esc_url( $listing_url ); ?>">
					...
					<div class="row">
						<div class="col-lg-12 col-md-12 col-sm-12">
							<input type="submit" class="btn theme-bg rounded full-width" value="<?php echo esc_html__( $instance['submit_button'], 'clasify-classified-listing' ); ?>" />
						</div>
					</div>
				</form>
			...
			echo '<div style="text-align:center;font-size:12px;display:flex;flex-direction:column;gap:10px;"><img width="100%" src="' . CLASIFY_CLASSIFIED_PLUGIN_ASSETS . '/img/search.svg"><p>Listing search placeholder. Check the preview on frontend.</p></div>';
		}
	...
	public function form( $instance ) {
		...
		$submit_button = ! empty( $instance['submit_button'] ) ? $instance['submit_button'] : esc_html__( 'Search', 'clasify-classified-listing' );
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php echo esc_html__( 'Title', 'clasify-classified-listing' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		</p>
		<!-- Submit Button -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'submit_button' ) ); ?>"><?php echo esc_html__( 'Submit Button', 'clasify-classified-listing' ); ?></label>
