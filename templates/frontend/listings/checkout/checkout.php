<div id="cl_checkout_wrap">
	<?php if ( CCP()->front->cart->cl_get_cart_contents() || CCP()->front->cart->cl_cart_has_fees() ) {
		echo CCP()->front->cart->cl_shopping_cart();
		?>
		<div id="cl_checkout_form_wrap" class="cl_clearfix">
			...
			<form id="cl_purchase_form" class="cl_form" action="<?php echo esc_attr( $form_action ); ?>" method="POST">
				...
				if ( cl_is_ajax_disabled() && ! empty( $_REQUEST['payment-mode'] ) ) {
				...
				} elseif ( CCP()->front->gateways->cl_show_gateways() ) {
					?>
					<fieldset id="cl_payment_mode_select">
						<legend><?php _e( 'Select Payment Method', 'clasify-classified-listing' ); ?></legend>
					...
					<fieldset id="cl_payment_mode_submit" class="cl-no-js">
						<p id="cl-next-submit-wrap">
							...
							<input type="submit" name="gateway_submit" id="cl_next_button" class="cl-submit" value="<?php _e( 'Next', 'clasify-classified-listing' ); ?>" />
						</p>
					</fieldset>
					<?php
				} else {
