		...
		if ( isset( $args['field_data']['value'] ) && ! empty( $args['field_data']['value'] ) ) { // --- WITH VALUE --- //
			if ( $clone_data == false ) {
				...
				foreach ( $args['field_data']['value'] as $c_f_v ) {
					...
					echo '<button class="remove_clone_btn"><span class="dashicons dashicons-dismiss"></span> ' . esc_html__( 'Remove', 'clasify-classified-listing' ) . '</button></div>';
				}

				echo '<button id="' . esc_attr( $args['id'] ) . '" class="mb_btn primary clone_group_btn"><span class="dashicons dashicons-plus-alt"></span>' . esc_html__( 'Add New', 'clasify-classified-listing' ) . '</button>';
				echo '</div>';
			}
