...
if ( ! is_user_logged_in() ) {
	echo '<p>' . esc_html__( 'Please', 'clasify-classified-listing' ) . ' <a href="' . esc_url( get_page_link( cl_admin_get_option( 'login_redirect_page' ) ) ) . '">' . esc_html__( 'Login', 'clasify-classified-listing' ) . '</a></p>';
} else {
	wp_enqueue_media();
	global $current_user;
	...
	if ( isset( $_POST['submit'] ) ) {
		...
		if ( strcmp( $password, $conf_password ) !== 0 ) {
			$error['error_msg'] = esc_html__( "Password didn't match", 'clasify-classified-listing' );
		}
		...
		if ( count( $error ) == 0 ) {
			...
			if ( ! is_wp_error( $user_update ) ) {
				$alert['class'] = 'success';
				$alert['msg']   = esc_html__( 'Profile Successfully Updated.', 'clasify-classified-listing' );
			}
		} else {
			$alert['class'] = 'danger';
			...
			<div class="row">
				<div class="col-md-12 form-group cl_user_avatar">
					...
					<label for="file-input" class="frontend-avatar select_single_label">
						<i class="fa fa-upload"></i><?php esc_html_e( ' Select Image', 'clasify-classified-listing' ); ?>
					</label>
					<input class="single_img_id" name="user_avt" type="hidden" />
				</div>
				<div class="col-md-12 form-group">
					<label for="user_name"><?php esc_html_e( 'User Name', 'clasify-classified-listing' ); ?></label>
