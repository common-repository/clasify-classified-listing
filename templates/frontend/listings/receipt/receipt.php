...
if ( empty( $payment ) ) : ?>
	<div class="cl_errors cl-alert cl-alert-error">
		<?php _e( 'The specified receipt ID appears to be invalid', 'clasify-classified-listing' ); ?>
	</div>
	<?php
	return;
endif;
...
<table id="cl_purchase_receipt" class="cl-table">
	<thead>
		...
		<?php if ( filter_var( $cl_receipt_args['payment_id'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<th><strong><?php _e( 'Payment', 'clasify-classified-listing' ); ?>:</strong></th>
	...
	<tbody>
		...
		<tr>
			<td class="cl_receipt_payment_status"><strong><?php _e( 'Payment Status', 'clasify-classified-listing' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $cl_receipt_args['payment_key'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php _e( 'Payment Key', 'clasify-classified-listing' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $cl_receipt_args['payment_method'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php _e( 'Payment Method', 'clasify-classified-listing' ); ?>:</strong></td>
				<td><?php echo CCP()->front->gateways->cl_get_gateway_checkout_label( cl_get_payment_gateway( $payment->ID ) ); ?></td>
			</tr>
		<?php endif; ?>
		<?php if ( filter_var( $cl_receipt_args['date'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php _e( 'Date', 'clasify-classified-listing' ); ?>:</strong></td>
		...
		<?php if ( ( $fees = cl_get_payment_fees( $payment->ID, 'fee' ) ) ) : ?>
			<tr>
				<td><strong><?php _e( 'Fees', 'clasify-classified-listing' ); ?>:</strong></td>
				<td>
					<ul class="cl_receipt_fees">
						<?php foreach ( $fees as $fee ) : ?>
							<li>
								...
								<span class="cl_fee_amount"><?php echo CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $fee['amount'] ) ); ?></span>
							</li>
						<?php endforeach; ?>
					</ul>
				</td>
		...
		<?php if ( filter_var( $cl_receipt_args['discount'], FILTER_VALIDATE_BOOLEAN ) && isset( $user['discount'] ) && $user['discount'] != 'none' ) : ?>
			<tr>
				<td><strong><?php _e( 'Discount(s)', 'clasify-classified-listing' ); ?>:</strong></td>
		...
		<?php if ( CCP()->front->tax->cl_use_taxes() ) : ?>
			<tr>
				<td><strong><?php _e( 'Tax', 'clasify-classified-listing' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $cl_receipt_args['price'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			...
			<tr>
				<td><strong><?php _e( 'Subtotal', 'clasify-classified-listing' ); ?>:</strong></td>
				<td>
			...
			<tr>
				<td><strong><?php _e( 'Total Price', 'clasify-classified-listing' ); ?>:</strong></td>
...
<?php if ( filter_var( $cl_receipt_args['products'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
	...
	<h3><?php echo apply_filters( 'cl_payment_receipt_products_title', __( 'Products', 'clasify-classified-listing' ) ); ?></h3>

	<table id="cl_purchase_receipt_products" class="cl-table">
		<thead>
			<th><?php _e( 'Name', 'clasify-classified-listing' ); ?></th>
			<?php if ( CCP()->common->options->cl_use_skus() ) { ?>
				<th><?php _e( 'SKU', 'clasify-classified-listing' ); ?></th>
			<?php } ?>
			<?php if ( cl_item_quantities_enabled() ) : ?>
				<th><?php _e( 'Quantity', 'clasify-classified-listing' ); ?></th>
			<?php endif; ?>
			<th><?php _e( 'Price', 'clasify-classified-listing' ); ?></th>
		</thead>
						...
						<tr>
							<td>
								...
								<div class="cl_purchase_receipt_product_name">
									...
									<?php if ( CCP()->front->listing_provider->cl_has_variable_prices( $item['id'] ) && ! is_null( $price_id ) ) : ?>
								...
								<?php
								$notes = CCP()->front->listingsaction->cl_get_product_notes( $item['id'] );
							...
							<td>
								...
								if ( empty( $item['in_bundle'] ) ) : // Only show price when product is not part of a bundle
									?>
									<?php echo CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $item['price'] ) ); ?>
								<?php endif; ?>
							</td>
						</tr>
					<?php endif; ?>
				...
				<?php foreach ( $fees as $fee ) : ?>
					<tr>
						...
						<?php if ( cl_item_quantities_enabled() ) : ?>
							<td></td>
						<?php endif; ?>
						<td class="cl_fee_amount"><?php echo CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $fee['amount'] ) ); ?></td>
					</tr>
				<?php endforeach; ?>
			<?php endif; ?>
		</tbody>
