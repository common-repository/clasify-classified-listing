...
if($woocommerce_active == '1' && class_exists( 'WooCommerce' )){
	$price = get_post_meta($listing->ID,'clasify_classified_plugin_pricing',true);
...
<form id="<?php echo esc_attr( $form_id ); ?>" class="cl_listing_purchase_form cl_purchase_<?php echo absint( esc_attr( $listing->ID ) ); ?>" method="post">
	<div class="cl_purchase_submit_wrapper">
		...
		if ( ! cl_is_ajax_disabled() ) {
			$timestamp = time();
			echo '<a href="#" class="cl-add-to-cart ' . esc_attr( $class ) . '" data-nonce="' . wp_create_nonce( 'cl-add-to-cart-' . esc_attr( $listing->ID ) ) . '" data-timestamp="' . esc_attr( $timestamp ) . '" data-token="' . esc_attr( \Clasify\Classified\Front\Purchase\Tokenizer\Tokenizer::tokenize( $timestamp ) ) . '" data-action="cl_add_to_cart" data-listing-id="' . esc_attr( $listing->ID ) . '" ' . $data_variable . ' ' . $type . ' ' . $data_price . ' ' . $button_display . '><span class="cl-add-to-cart-label">' . $args['text'] . '</span> <span class="cl-loading" aria-label="' . esc_attr__( 'Loading', 'clasify-classified-listing' ) . '"></span></a>';
		}
		...
		echo '<a href="' . esc_url( cl_get_checkout_uri() ) . '" class="cl_go_to_checkout ' . esc_attr( $class ) . '" ' . $checkout_display . '>' . $args['checkout'] . '</a>';
		...
		<?php if ( ! cl_is_ajax_disabled() ) : ?>
			<span class="cl-cart-ajax-alert" aria-live="assertive">
				<span class="cl-cart-added-alert" style="display: none;">
					...
					<?php echo __( 'Added to cart', 'clasify-classified-listing' ); ?>
				</span>
			</span>
		<?php endif; ?>
		<?php if ( ! $listing->is_free && ! cl_listing_is_tax_exclusive( $listing->ID ) ) : ?>
			<?php
			if ( cl_display_tax_rate() && CCP()->front->tax->cl_prices_include_tax() ) {
				echo '<span class="cl_purchase_tax_rate">' . sprintf( __( 'Includes %1$s&#37; tax', 'clasify-classified-listing' ), cl_get_tax_rate() * 100 ) . '</span>';
			} elseif ( cl_display_tax_rate() && ! CCP()->front->tax->cl_prices_include_tax() ) {
				echo '<span class="cl_purchase_tax_rate">' . sprintf( __( 'Excluding %1$s&#37; tax', 'clasify-classified-listing' ), cl_get_tax_rate() * 100 ) . '</span>';
			}
			?>
		<?php endif; ?>
	</div>
	...
	<?php if ( apply_filters( 'cl_listing_redirect_to_checkout', CCP()->common->options->cl_straight_to_checkout(), $listing->ID, $args ) ) : ?>
