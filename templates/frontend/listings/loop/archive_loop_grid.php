 ...
 *
 * This template can be overridden by copying it to yourtheme/clasify-classified-listing/archive-cl_cpt.php.
 *
 * HOWEVER, on occasion clasify-classified-listing will need to update template files and you
 * (the theme developer) will need to copy the new files to your theme to
 * maintain compatibility. We try to do this as little as possible, but it does
 * happen. When this occurs the version of the template file will be bumped and
 * the readme will list any important changes.
 *
 * @see     https://docs.clasify-classified-listing.com/document/template-structure/
 * @package clasify-classified-listing/Templates
 * @version 1.0.0
 */
...
// START MAP DATA
$img_url      = ! empty( get_the_post_thumbnail_url() ) ? get_the_post_thumbnail_url() : CLASIFY_CLASSIFIED_PLUGIN_ASSETS . '/img/placeholder_light.png';
...
if ( empty( $archive_setting ) || $archive_setting == 'null' ) {
	$fields = array(
		...
		'thumbnail_section' => array(
			'topleft'     => array( 'listing_category' ),
			'topright'    => array(),
			'bottomleft'  => array(),
		...
		'sectionone'        => array(
			'sectiononeleft'  => array( 'listing_category' ),
			...
			foreach ( $asettings as $keyname => $asetting ) {
				if ( is_array( $asetting ) ) {
					if ( $asetting['active'] == 1 ) {
						$fields['thumbnail_section'][ $key ][] = 'listing_category';
					}
				}
			}
		}
...
<div <?php post_class( $listing_columns . ' col-sm-12 listing_data' ); ?> id="post-<?php the_ID(); ?>" data-listing="<?php echo esc_attr( $value ); ?>">
	<div class="property-listing property-grid">
		...
		<div class="thumbnail-section lazy-section">
			<div class="clasify-classified-plugin-thumb-sec top-left"><?php $provider->render_loop_sections( $fields['thumbnail_section']['topleft'] ); ?></div>
			<div class="clasify-classified-plugin-thumb-sec top-right"><?php $provider->render_loop_sections( $fields['thumbnail_section']['topright'] ); ?></div>
			<div class="clasify-classified-plugin-thumb-sec bottom-left"><?php $provider->render_loop_sections( $fields['thumbnail_section']['bottomleft'] ); ?></div>
			<div class="clasify-classified-plugin-thumb-sec bottom-right"><?php $provider->render_loop_sections( $fields['thumbnail_section']['bottomright'] ); ?></div>
