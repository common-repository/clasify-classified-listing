...
<div class="wrap cl-wrap">
	<h2><?php printf( __( 'Payment %s', 'clasify-classified-listing' ), $number ); ?></h2>
					...
					<div id="postbox-container-1" class="postbox-container">
						<div id="side-sortables" class="meta-box-sortables ui-sortable">
							...
							<div id="cl-order-update" class="postbox cl-order-data">
								...
								<h3 class="hndle">
									<span><?php _e( 'Update Payment', 'clasify-classified-listing' ); ?></span>
								</h3>
								<div class="inside">
									<div class="cl-admin-box">
										...
										<div class="cl-admin-box-inside">
											<p>
												<span class="label"><?php _e( 'Status:', 'clasify-classified-listing' ); ?></span>&nbsp;
												...
												<?php
												$status_help  = '<ul>';
												$status_help .= '<li>' . __( '<strong>Pending</strong>: payment is still processing or was abandoned by customer. Successful payments will be marked as Complete automatically once processing is finalized.', 'clasify-classified-listing' ) . '</li>';
												$status_help .= '<li>' . __( '<strong>Complete</strong>: all processing is completed for this purchase.', 'clasify-classified-listing' ) . '</li>';
												$status_help .= '<li>' . __( '<strong>Revoked</strong>: access to purchased items is disabled, perhaps due to policy violation or fraud.', 'clasify-classified-listing' ) . '</li>';
												$status_help .= '<li>' . __( '<strong>Refunded</strong>: the purchase amount is returned to the customer and access to items is disabled.', 'clasify-classified-listing' ) . '</li>';
												$status_help .= '<li>' . __( '<strong>Abandoned</strong>: the purchase attempt was not completed by the customer.', 'clasify-classified-listing' ) . '</li>';
												$status_help .= '<li>' . __( '<strong>Failed</strong>: customer clicked Cancel before completing the purchase.', 'clasify-classified-listing' ) . '</li>';
												$status_help .= '</ul>';
												?>
											...
											<?php if ( $payment->is_recoverable() ) : ?>
												<p>
													<span class="label"><?php _e( 'Recovery URL', 'clasify-classified-listing' ); ?>:</span>
													<?php $recover_help = __( 'Pending and abandoned payments can be resumed by the customer, using this custom URL. Payments can be resumed only when they do not have a transaction ID from the gateway.', 'clasify-classified-listing' ); ?>
										...
										<div class="cl-admin-box-inside">
											<p>
												<span class="label"><?php _e( 'Date:', 'clasify-classified-listing' ); ?></span>&nbsp;
										...
										<div class="cl-admin-box-inside">
											<p>
												<span class="label"><?php _e( 'Time:', 'clasify-classified-listing' ); ?></span>&nbsp;
											...
											<p>
												...
												if ( 'none' !== $payment->discounts ) {
													...
													foreach ( $discounts as $discount ) {
														$discount_obj = CCP()->admin->discount_action->cl_get_discount_by_code( $discount );
												...
												?>
												<span class="label">
													<?php echo _n( 'Discount Code', 'Discount Codes', count( $found_discounts ), 'clasify-classified-listing' ); ?>:
												</span>&nbsp;
												<span>
													<?php
													if ( ! empty( $found_discounts ) ) {
														...
														_e( 'None', 'clasify-classified-listing' );
													}
													?>
												</span>
											</p>
										...
										if ( ! empty( $fees ) ) :
											...
											<div class="cl-order-fees cl-admin-box-inside">
												<p class="strong"><?php _e( 'Fees', 'clasify-classified-listing' ); ?>:</p>
												<ul class="cl-payment-fees">
													<?php foreach ( $fees as $fee ) : ?>
														<li data-fee-id="<?php echo esc_attr( $fee['id'] ); ?>"><span class="fee-label"><?php echo esc_html( $fee['label'] ) . ':</span> ' . '<span class="fee-amount" data-fee="' . esc_attr( $fee['amount'] ) . '">' . CCP()->common->formatting->cl_currency_filter( $fee['amount'], $currency_code ); ?></span></li>
													<?php endforeach; ?>
												</ul>
											</div>
										<?php endif; ?>
										...
										if ( $ret ) :
											...
											<div class="cl-order-taxes cl-admin-box-inside">
												<p>
													<span class="label"><?php _e( 'Tax', 'clasify-classified-listing' ); ?>:</span>&nbsp;
													<input name="cl-payment-tax" class="med-text" type="text" value="<?php echo esc_attr( CCP()->common->formatting->cl_format_amount( $payment->tax ) ); ?>" />
										...
										<div class="cl-order-payment cl-admin-box-inside">
											<p>
												<span class="label"><?php _e( 'Total Price', 'clasify-classified-listing' ); ?>:</span>&nbsp;
												<?php echo CCP()->common->currencies->cl_currency_symbol( $payment->currency ); ?>&nbsp;<input name="cl-payment-total" type="text" class="med-text" value="<?php echo esc_attr( CCP()->common->formatting->cl_format_amount( $payment->total ) ); ?>" />
											</p>
										</div>

										<div class="cl-order-payment-recalc-totals cl-admin-box-inside" style="display:none">
											<p>
												<span class="label"><?php _e( 'Recalculate Totals', 'clasify-classified-listing' ); ?>:</span>&nbsp;
												<a href="" id="cl-order-recalc-total" class="button button-secondary right"><?php _e( 'Recalculate', 'clasify-classified-listing' ); ?></a>
											</p>
										</div>
								...
								<div class="cl-order-update-box cl-admin-box">
									...
									<div id="major-publishing-actions">
										<div id="delete-action">
											...
											?>
											" class="cl-delete-payment cl-delete"><?php _e( 'Delete Payment', 'clasify-classified-listing' ); ?></a>
										</div>
										<input type="submit" class="button button-primary right" value="<?php esc_attr_e( 'Save Payment', 'clasify-classified-listing' ); ?>" />
										<div class="clear"></div>
									</div>
								...
								<div id="cl-order-resend-receipt" class="postbox cl-order-data">
									<div class="inside">
										<div class="cl-order-resend-receipt-box cl-admin-box">
											...
											<a href="<?php echo esc_url( $email_links ); ?>" id="<?php echo esc_attr( $email_id ); ?>" class="button-secondary alignleft"><?php _e( 'Resend Receipt', 'clasify-classified-listing' ); ?></a>
											<span alt="f223" class="cl-help-tip dashicons dashicons-editor-help" title="<?php _e( '<strong>Resend Receipt</strong>: This will send a new copy of the purchase receipt to the customer&#8217;s email address. If listing URLs are included in the receipt, new file listing URLs will also be included with the receipt.', 'clasify-classified-listing' ); ?>"></span>
											<?php if ( count( $customer->emails ) > 1 ) : ?>
												...
												<div class="cl-order-resend-receipt-addresses" style="display:none;">
													<select class="cl-order-resend-receipt-email">
														<option value=""><?php _e( ' -- select email --', 'clasify-classified-listing' ); ?></option>
							...
							<div id="cl-order-details" class="postbox cl-order-data">
								...
								<h3 class="hndle">
									<span><?php _e( 'Payment Meta', 'clasify-classified-listing' ); ?></span>
								</h3>
								<div class="inside">
									<div class="cl-admin-box">
										...
										if ( $gateway ) :
											...
											<div class="cl-order-gateway cl-admin-box-inside">
												<p>
													<span class="label"><?php _e( 'Gateway:', 'clasify-classified-listing' ); ?></span>&nbsp;
										...
										<div class="cl-order-payment-key cl-admin-box-inside">
											<p>
												<span class="label"><?php _e( 'Key:', 'clasify-classified-listing' ); ?></span>&nbsp;
										...
										<div class="cl-order-ip cl-admin-box-inside">
											<p>
												<span class="label"><?php _e( 'IP:', 'clasify-classified-listing' ); ?></span>&nbsp;
										...
										<?php if ( $transaction_id ) : ?>
											<div class="cl-order-tx-id cl-admin-box-inside">
												<p>
													<span class="label"><?php _e( 'Transaction ID:', 'clasify-classified-listing' ); ?></span>&nbsp;
										...
										<div class="cl-unlimited-listings cl-admin-box-inside">
											<p>
												...
												<label class="description" for="cl_unlimited_listings"><?php _e( 'Unlimited file listings', 'clasify-classified-listing' ); ?></label>
												<span alt="f223" class="cl-help-tip dashicons dashicons-editor-help" title="<?php _e( '<strong>Unlimited file listings</strong>: checking this box will override all other file listing limits for this purchase, granting the customer unliimited listings of all files included on the purchase.', 'clasify-classified-listing' ); ?>"></span>
											</p>
										</div>
							...
							<div id="cl-order-logs" class="postbox cl-order-logs">
								...
								<h3 class="hndle">
									<span><?php _e( 'Logs', 'clasify-classified-listing' ); ?></span>
								</h3>
								<div class="inside">
									<div class="cl-admin-box">
										...
										<div class="cl-admin-box-inside">
											...
											<p>
												<a href="<?php echo admin_url( '/edit.php?post_type=listing&page=cl-reports&tab=logs&payment=' . esc_attr( $payment_id ) ); ?>"><?php _e( 'View file listing log for purchase', 'clasify-classified-listing' ); ?></a>
											</p>
											<p>
												<?php $listing_log_url = admin_url( 'edit.php?post_type=listing&page=cl-reports&tab=logs&customer=' . esc_attr( $customer->id ) ); ?>
												<a href="<?php echo esc_url( $listing_log_url ); ?>"><?php _e( 'View customer listing log', 'clasify-classified-listing' ); ?></a>
											</p>
											<p>
												...
												<a href="<?php echo esc_url( $purchase_url ); ?>"><?php _e( 'View all purchases of customer', 'clasify-classified-listing' ); ?></a>
											</p>
										</div>
					...
					<div id="postbox-container-2" class="postbox-container">
						<div id="normal-sortables" class="meta-box-sortables ui-sortable">
							...
							if ( empty( $cart_items ) ) {
								$cart_items = array(
									array(
										'name'        => __( 'No listing attached to this order', 'clasify-classified-listing' ),
										'id'          => 0,
										'item_number' => array(
											'id'       => 0,
											'quantity' => 0,
							...
							if ( is_array( $cart_items ) ) :
								...
								<div id="cl-purchased-files" class="postbox cl-edit-purchase-element <?php echo esc_attr( $column_count ); ?>">
									<h3 class="hndle cl-payment-details-label-mobile">
										<span><?php printf( __( 'Purchased %s', 'clasify-classified-listing' ), cl_get_label_plural() ); ?></span>
									</h3>
									<div class="cl-purchased-files-header row header">
										<ul class="cl-purchased-files-list-wrapper">
											<li class="listing">
												<?php printf( _x( '%s Purchased', 'payment details purchased item title - full screen', 'clasify-classified-listing' ), cl_get_label_singular() ); ?>
											</li>

											<li class="item_price<?php echo esc_attr( $is_qty_enabled ); ?>">
												<?php
												_ex( 'Price', 'payment details purchased item price - full screen', 'clasify-classified-listing' );
												if ( cl_item_quantities_enabled() ) :
													_ex( ' & Quantity', 'payment details purchased item quantity - full screen', 'clasify-classified-listing' );
												endif;
												?>
											</li>
											...
											<?php if ( $taxs->cl_use_taxes() ) : ?>
												<li class="item_tax">
													<?php _ex( 'Tax', 'payment details purchased item tax - full screen', 'clasify-classified-listing' ); ?>
												</li>
											<?php endif; ?>

											<li class="price">
												<?php printf( _x( '%s Total', 'payment details purchased item total - full screen', 'clasify-classified-listing' ), cl_get_label_singular() ); ?>
											</li>
										</ul>
									</div>
									<?php
											...
											<ul class="cl-purchased-files-list-wrapper">
														...
														<?php else : ?>
															<span class="deleted">
																...
																if ( ! $cart_items_existed ) {
																	...
																	?>
																	&nbsp;-&nbsp;
																<em>(<?php _e( 'Deleted', 'clasify-classified-listing' ); ?>)</em>
															<?php } else { ?>
																<em><?php printf( __( '%s deleted', 'clasify-classified-listing' ), cl_get_label_singular() ); ?></em>
															<?php } ?>
															</span>
														<?php endif; ?>
													</span>
												...
												<li class="item_price<?php echo esc_attr( $is_qty_enabled ); ?>">
													<span class="cl-payment-details-label-mobile">
														<?php
														_ex( 'Price', 'payment details purchased item price - mobile', 'clasify-classified-listing' );
														if ( cl_item_quantities_enabled() ) :
															_ex( ' & Quantity', 'payment details purchased item quantity - mobile', 'clasify-classified-listing' );
														endif;
														?>
													</span>
													<?php echo CCP()->common->currencies->cl_currency_symbol( $currency_code ); ?>
													<input type="text" class="medium-text cl-price-field cl-payment-details-listing-item-price cl-payment-item-input" name="cl-payment-details-listings[<?php echo esc_attr( $key ); ?>][item_price]" value="<?php echo CCP()->common->formatting->cl_format_amount( $item_price ); ?>" />
													<?php if ( cl_item_quantities_enabled() ) : ?>
														&nbsp;&times;&nbsp;
												...
												<?php if ( $taxs->cl_use_taxes() ) : ?>
													<li class="item_tax">
														<span class="cl-payment-details-label-mobile">
															<?php _ex( 'Tax', 'payment details purchased item tax - mobile', 'clasify-classified-listing' ); ?>
														</span>
														<?php echo CCP()->common->currencies->cl_currency_symbol( $currency_code ); ?>
														<input type="text" class="small-text cl-price-field cl-payment-details-listing-item-tax cl-payment-item-input" name="cl-payment-details-listings[<?php echo esc_attr( $key ); ?>][item_tax]" value="<?php echo CCP()->common->formatting->cl_format_amount( $item_tax ); ?>" />
													</li>
												<?php endif; ?>

												<li class="price">
													<span class="cl-payment-details-label-mobile">
														<?php printf( _x( '%s Total', 'payment details purchased item total - mobile', 'clasify-classified-listing' ), cl_get_label_singular() ); ?>
													</span>
													<span class="cl-price-currency"><?php echo CCP()->common->currencies->cl_currency_symbol( $currency_code ); ?></span><span class="price-text cl-payment-details-listing-amount"><?php echo CCP()->common->formatting->cl_format_amount( $price ); ?></span>
								...
								?>
								<div class="row">
									<p><?php printf( __( 'No %s included with this purchase', 'clasify-classified-listing' ), cl_get_label_plural() ); ?></p>
								</div>
							<?php endif; ?>
							...
							<div id="cl-customer-details" class="postbox">
								<h3 class="hndle">
									<span><?php _e( 'Customer Details', 'clasify-classified-listing' ); ?></span>
								</h3>
								<div class="inside cl-clearfix">
									...
									<div class="column-container customer-info">
										...
										<div class="column">
											<a href="#change" class="cl-payment-change-customer"><?php _e( 'Assign to another customer', 'clasify-classified-listing' ); ?></a>
											&nbsp;|&nbsp;
											<a href="#new" class="cl-payment-new-customer"><?php _e( 'New Customer', 'clasify-classified-listing' ); ?></a>
										</div>
									</div>

									<div class="column-container change-customer" style="display: none">
										<div class="column">
											<strong><?php _e( 'Select a customer', 'clasify-classified-listing' ); ?>:</strong>
											<?php
											$args = array(
												'class'    => 'cl-payment-change-customer-input',
												'selected' => $customer->id,
												'name'     => 'customer-id',
												'placeholder' => __( 'Type to search all Customers', 'clasify-classified-listing' ),
											);

											echo CCP()->common->dbcustomer->customer_dropdown( $args );
										...
										<div class="column">
											<strong><?php _e( 'Actions', 'clasify-classified-listing' ); ?>:</strong>
											<br />
											<input type="hidden" id="cl-change-customer" name="cl-change-customer" value="0" />
											<a href="#cancel" class="cl-payment-change-customer-cancel cl-delete"><?php _e( 'Cancel', 'clasify-classified-listing' ); ?></a>
										</div>
										<div class="column">
											<small><em>*<?php _e( 'Click "Save Payment" to change the customer', 'clasify-classified-listing' ); ?></em></small>
										</div>
									</div>

									<div class="column-container new-customer" style="display: none">
										<div class="column">
											<strong><?php _e( 'Name', 'clasify-classified-listing' ); ?>:</strong>&nbsp;
											<input type="text" name="cl-new-customer-name" value="" class="medium-text" />
										</div>
										<div class="column">
											<strong><?php _e( 'Email', 'clasify-classified-listing' ); ?>:</strong>&nbsp;
										...
										<div class="column">
											...
											<a href="#cancel" class="cl-payment-new-customer-cancel cl-delete"><?php _e( 'Cancel', 'clasify-classified-listing' ); ?></a>
										</div>
										<div class="column">
											<small><em>*<?php _e( 'Click "Save Payment" to create new customer', 'clasify-classified-listing' ); ?></em></small>
										</div>
									</div>
							...
							<div id="cl-billing-details" class="postbox">
								<h3 class="hndle">
									<span><?php _e( 'Billing Address', 'clasify-classified-listing' ); ?></span>
								</h3>
								<div class="inside cl-clearfix">
									...
									<div id="cl-order-address">
										...
										<div class="order-data-address">
											<div class="data column-container">
												<div class="column">
													<p>
														<strong class="order-data-address-line"><?php _e( 'Street Address Line 1:', 'clasify-classified-listing' ); ?></strong><br />
														<input type="text" name="cl-payment-address[0][line1]" value="<?php echo esc_attr( $address['line1'] ); ?>" class="large-text" />
													</p>
													<p>
														<strong class="order-data-address-line"><?php _e( 'Street Address Line 2:', 'clasify-classified-listing' ); ?></strong><br />
												...
												<div class="column">
													<p>
														<strong class="order-data-address-line"><?php echo _x( 'City:', 'Address City', 'clasify-classified-listing' ); ?></strong><br />
														<input type="text" name="cl-payment-address[0][city]" value="<?php echo esc_attr( $address['city'] ); ?>" class="large-text" />

													</p>
													<p>
														<strong class="order-data-address-line"><?php echo _x( 'Zip / Postal Code:', 'Zip / Postal code of address', 'clasify-classified-listing' ); ?></strong><br />
												...
												<div class="column">
													<p id="cl-order-address-country-wrap">
														<strong class="order-data-address-line"><?php echo _x( 'Country:', 'Address country', 'clasify-classified-listing' ); ?></strong><br />
														<?php
														echo CCP()->admin->settings_instances->cl_admin_select_callback(
															array(
																'options'          => CCP()->front->country->cl_get_country_list(),
																...
																'chosen'           => true,
																'placeholder'      => __( 'Select a country', 'clasify-classified-listing' ),
																'data'             => array(
																	'search-type'        => 'no_ajax',
																	'search-placeholder' => __( 'Type to search all Countries', 'clasify-classified-listing' ),
													...
													<p id="cl-order-address-state-wrap">
														<strong class="order-data-address-line"><?php echo _x( 'State / Province:', 'State / province of address', 'clasify-classified-listing' ); ?></strong><br />
														<?php
														$states = CCP()->front->country->cl_get_shop_states( $address['country'] );
														if ( ! empty( $states ) ) {
															echo CCP()->admin->settings_instances->cl_admin_select_callback(
																array(
																	'options'          => $states,
																	...
																	'show_option_none' => false,
																	'chosen'           => true,
																	'placeholder'      => __( 'Select a state', 'clasify-classified-listing' ),
																	'data'             => array(
																		'search-type'        => 'no_ajax',
																		'search-placeholder' => __( 'Type to search all States/Provinces', 'clasify-classified-listing' ),
																	),
																)
															);
														} else {
							...
							<div id="cl-payment-notes" class="postbox">
								<h3 class="hndle"><span><?php _e( 'Payment Notes', 'clasify-classified-listing' ); ?></span></h3>
								<div class="inside">
									<div id="cl-payment-notes-inner">
										...
										echo '<p class="cl-no-payment-notes"' . $no_notes_display . '>' . __( 'No payment notes', 'clasify-classified-listing' ) . '</p>';
										?>
									</div>
									<textarea name="cl-payment-note" id="cl-payment-note" class="large-text"></textarea>

									<p>
										<button id="cl-add-payment-note" class="button button-secondary right" data-payment-id="<?php echo absint( $payment_id ); ?>"><?php _e( 'Add Note', 'clasify-classified-listing' ); ?></button>
									</p>
