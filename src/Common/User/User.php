<?php
namespace Clasify\Classified\Common\User;

use Clasify\Classified\Traitval\Traitval;
use Clasify\Classified\Front\Purchase\Payments\Clpaymentsquery;
...
class User {
				...
				if ( is_array( $purchased_files ) ) {
					foreach ( $purchased_files as $listing ) {
						if ( in_array( $listing['id'], $listings ) ) {
							$variable_prices = CCP()->front->listing_provider->cl_has_variable_prices( $listing['id'] );
	...
	function cl_get_purchase_stats_by_user( $user = '' ) {
		...
		$stats    = array();
		$customer = CCP()->common->dbcustomer->get_customer_by( $field, $user );
		...
		if ( $customer ) {
			...
			$stats['total_spent'] = CCP()->common->formatting->cl_sanitize_amount( $customer->purchase_value );
		}
	...
	function cl_count_total_customers( $args = array() ) {
		return CCP()->common->dbcustomer->count( $args );
	}
	...
	function cl_new_user_notification( $user_id = 0, $user_data = array() ) {
		...
		$admin_subject  = apply_filters( 'cl_user_registration_admin_email_subject', sprintf( __( '[%s] New User Registration', 'clasify-classified-listing' ), $from_name ), $user_data );
		$admin_heading  = apply_filters( 'cl_user_registration_admin_email_heading', __( 'New user registration', 'clasify-classified-listing' ), $user_data );
		$admin_message  = sprintf( __( 'Username: %s', 'clasify-classified-listing' ), $user_data['user_login'] ) . "\r\n\r\n";
		$admin_message .= sprintf( __( 'E-mail: %s', 'clasify-classified-listing' ), $user_data['user_email'] ) . "\r\n";
		...
		$user_subject = apply_filters( 'cl_user_registration_email_subject', sprintf( __( '[%s] Your username and password', 'clasify-classified-listing' ), $from_name ), $user_data );
		$user_heading = apply_filters( 'cl_user_registration_email_heading', __( 'Your account info', 'clasify-classified-listing' ), $user_data );
		$user_message = apply_filters( 'cl_user_registration_email_username', sprintf( __( 'Username: %s', 'clasify-classified-listing' ), $user_data['user_login'] ) . "\r\n", $user_data );

		if ( did_action( 'cl_pre_process_purchase' ) ) {
			$password_message = __( 'Password entered at checkout', 'clasify-classified-listing' );
		} else {
			$password_message = __( 'Password entered at registration', 'clasify-classified-listing' );
		}

		$user_message .= apply_filters( 'cl_user_registration_email_password', sprintf( __( 'Password: %s', 'clasify-classified-listing' ), '[' . $password_message . ']' ) . "\r\n" );

		$login_url = apply_filters( 'cl_user_registration_email_login_url', wp_login_url() );
		if ( $emails->html ) {

			$user_message .= '<a href="' . esc_url( $login_url ) . '"> ' . esc_attr__( 'Click here to log in', 'clasify-classified-listing' ) . ' &raquo;</a>' . "\r\n";
		} else {

			$user_message .= sprintf( __( 'To log in, visit: %s', 'clasify-classified-listing' ), $login_url ) . "\r\n";
		}
	...
	function cl_send_user_verification_email( $user_id = 0 ) {
		...
		$from_name  = cl_admin_get_option( 'from_name', wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ) );
		$from_email = cl_admin_get_option( 'from_email', get_bloginfo( 'admin_email' ) );
		$subject    = apply_filters( 'cl_user_verification_email_subject', __( 'Verify your account', 'clasify-classified-listing' ), $user_id );
		$heading    = apply_filters( 'cl_user_verification_email_heading', __( 'Verify your account', 'clasify-classified-listing' ), $user_id );
		$message    = sprintf(
			__( "Hello %1\$s,\n\nYour account with %2\$s needs to be verified before you can access your purchase history. <a href='%3\$s'>Click here</a> to verify your account.\n\nLink missing? Visit the following URL: %4\$s", 'clasify-classified-listing' ),
			$name,
			$from_name,
			$url,
			$url
	...
	function cl_validate_user_verification_token( $url = '' ) {
		...
		if ( isset( $parts['query'] ) ) {
			...
			if ( isset( $query_args['ttl'] ) && current_time( 'timestamp' ) > $query_args['ttl'] ) {
				...
				$link_text = sprintf(
					__( 'Sorry but your account verification link has expired. <a href="%s">Click here</a> to request a new verification URL.', 'clasify-classified-listing' ),
					$this->cl_get_user_verification_request_url()
				);

				wp_die( apply_filters( 'cl_verification_link_expired_text', $link_text ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
			}
	...
	function cl_process_user_verification_request() {
		...
		if ( ! wp_verify_nonce( $_GET['_wpnonce'], 'cl-request-verification' ) ) {
			wp_die( __( 'Nonce verification failed.', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
		}

		if ( ! is_user_logged_in() ) {
			wp_die( __( 'You must be logged in to verify your account.', 'clasify-classified-listing' ), __( 'Notice', 'clasify-classified-listing' ), array( 'response' => 403 ) );
		}

		if ( ! $this->cl_user_pending_verification( get_current_user_id() ) ) {
			wp_die( __( 'Your account has already been verified.', 'clasify-classified-listing' ), __( 'Notice', 'clasify-classified-listing' ), array( 'response' => 403 ) );
		}
	...
	function cl_process_user_account_verification() {
		...
		if ( ! $this->cl_validate_user_verification_token( $url ) ) {
			...
			wp_die( __( 'Invalid verification token provided.', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
		}
