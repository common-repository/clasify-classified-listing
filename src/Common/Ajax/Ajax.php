<?php
namespace Clasify\Classified\Common\Ajax;
...
class Ajax {
	...
	function cl_enquiry_func() {
		if ( ! wp_verify_nonce( $_REQUEST['clasify_classified_enquiry'], 'clasify-classified-enquiry-form' ) ) {
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Nonce verification failed!', 'clasify-classified-listing' ),
				)
			);
		}
		...
		// $sent = wp_mail( $listing_email, $subject, strip_tags( $sent_message ));
		...
		if ( $sent ) {
			...
			if ( $insert_id ) {
				wp_send_json_success(
					array(
						'message' => esc_html__( 'Enquiry has been sent successfully!', 'clasify-classified-listing' ),
					)
				);
			}
		} //message sent!
			...
			wp_send_json_error(
				array(
					'message' => esc_html__( 'Failed to send message', 'clasify-classified-listing' ),
				)
			);
		} //message wasn't sent
	...
	function cl_ajax_remove_discount() {
		if ( isset( $_POST['code'] ) ) {
			...
			CCP()->front->discountaction->cl_unset_cart_discount( urldecode( cl_sanitization( $_POST['code'] ) ) );

			$total = CCP()->front->cart->cl_get_cart_total();

			$return = array(
				'total_plain' => $total,
				'total'       => html_entity_decode( CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $total ) ), ENT_COMPAT, 'UTF-8' ),
				'code'        => cl_sanitization( $_POST['code'] ),
				'discounts'   => CCP()->front->discountaction->cl_get_cart_discounts(),
				'html'        => CCP()->front->discountaction->cl_get_cart_discounts_html(),
			);
	...
	function cl_ajax_apply_discount() {
		if ( isset( $_POST['code'] ) ) {
			...
			if ( CCP()->front->discountaction->cl_is_discount_valid( $discount_code, $user ) ) {
				$discount  = CCP()->front->discountaction->cl_get_discount_by_code( $discount_code );
				$amount    = CCP()->front->discountaction->cl_format_discount_rate( CCP()->front->discountaction->cl_get_discount_type( $discount->ID ), CCP()->front->discountaction->cl_get_discount_amount( $discount->ID ) );
				$discounts = CCP()->front->discountaction->cl_set_cart_discount( $discount_code );
				$total     = CCP()->front->cart->cl_get_cart_total( $discounts );
				...
				$return = array(
					...
					'total'       => html_entity_decode( CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $total ) ), ENT_COMPAT, 'UTF-8' ),
					'code'        => $discount_code,
					'html'        => CCP()->front->discountaction->cl_get_cart_discounts_html( $discounts ),
				);
			} else {
				$errors        = CCP()->front->error->cl_get_errors();
				$return['msg'] = $errors['cl-discount-error'];
				CCP()->front->error->cl_unset_error( 'cl-discount-error' );
			}
	...
	function cl_ajax_get_states_field() {
		if ( empty( $_POST['country'] ) ) {
			$_POST['country'] = CCP()->front->country->cl_get_shop_country();
		}
		...
		if ( false !== $nonce_verified ) {
			$states = CCP()->front->country->cl_get_shop_states( cl_sanitization( $_POST['country'] ) );
			...
			if ( ! empty( $states ) ) {
				...
				$response = CCP()->admin->settings_instances->select( $args );
			} else {
	...
	function cl_ajax_remove_from_cart() {
		...
		if ( isset( $_POST['cart_item'] ) && isset( $_POST['nonce'] ) ) {
			...
			if ( false === $nonce_verified ) {
				...
				CCP()->front->cart->remove( $cart_item );

				$return = array(
					'removed'       => 1,
					'subtotal'      => html_entity_decode( CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( CCP()->front->cart->get_subtotal() ) ), ENT_COMPAT, 'UTF-8' ),
					'total'         => html_entity_decode( CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( CCP()->front->cart->get_total() ) ), ENT_COMPAT, 'UTF-8' ),
					'cart_quantity' => html_entity_decode( CCP()->front->cart->get_quantity() ),
				);
				if ( CCP()->front->tax->cl_use_taxes() ) {
					$cart_tax      = (float) CCP()->front->cart->get_tax();
					$return['tax'] = html_entity_decode( CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $cart_tax ) ), ENT_COMPAT, 'UTF-8' );
				}
			}
	...
	<?php if($field_type == 'textarea'){ ?>
		<div class="custom-field-item">
			...
			<textarea name="<?php echo $field_name;?>"><?php echo esc_attr( $field_default_value ) ?></textarea>
			<a href="javascript:void(0)" class="delete-field" data-field_name="<?php echo esc_attr($field_name);?>"><?php esc_html_e('Delete','clasify-classified-listing');?></a>
		</div>
	<?php }else{ ?>
		<div class="custom-field-item">
			<label><?php echo $field_label?></label>
			<input type="<?php echo $field_type;?>" name="<?php echo $field_name;?>" value="<?php echo esc_attr( $field_default_value ) ?>" disabled>
			<a class="delete-field" data-field_name="<?php echo esc_attr($field_name);?>"><?php esc_html_e('Delete','clasify-classified-listing');?></a>
		</div>
	<?php } ?>
	<?php }
		die();
	...
	public function cl_save_pagelayout_single_setting() {
		...
		$single_setting = CCP()->admin->settings_instances->admin_settings_extend( $single_setting, CCP()->admin->single_instance->single_default_layout_setting() );
	...
	public function cl_save_pagelayout_archive_setting() {
		...
		$archive_setting = CCP()->admin->settings_instances->admin_settings_extend( $archive_setting, CCP()->admin->archive_instance->cl_archive_default_grid_setting() );
	...
	public function listing_abuse_dialog_action_func() {
		...
		if ( is_object( get_post( $id ) ) && $data ) {
			...
			if ( $sent ) {
				...
				$return['message'] = esc_html__( 'Your message sent successfully.', 'clasify-classified-listing' );
				$return['success'] = true;
				$return['class']   = 'success';
			} else {
				$return['message'] = esc_html__( 'Something Wrong.', 'clasify-classified-listing' );
				$return['success'] = false;
				$return['class']   = 'error';
			}
		} else {
			$return['message'] = esc_html__( 'Please fill out the form.', 'clasify-classified-listing' );
			$return['class']   = 'error';
		}
	...
	public function cl_compare_func() {
		...
		foreach ( $comp_data as $post ) {
			$html .= '';
			$html .= "<div id=\"clasify-classified-plugin-compare-item{$post}\" class=\"compare-listing-single\"><div class=\"compare-item-img\">";
			if ( has_post_thumbnail( $post ) ) {
				$alt   = get_post_meta( $post, '_wp_attachment_image_alt', true );
				$html .= '<img src="' . esc_html( get_the_post_thumbnail_url( $post, 'thumbnail' ) ) . '" alt="' . esc_attr( $alt ) . '">';
			} else {
				$html .= '<img src="' . CLASIFY_CLASSIFIED_PLUGIN_ASSETS . '/img/placeholder_light.png' . '" alt="' . esc_attr__( 'Placeholder', 'clasify-classified-plugin' ) . '">';
			}
			$html .= '</div><div class="compare-item-content"><span class="item-title">';
			$html .= esc_html( get_the_title( $post ) );
			$html .= '</span>';
			$html .= '<a class="clasify-classified-plugin-compare-remove-btn" data-remove_compare_item="' . esc_attr( $post ) . '" href="javascript:void(0)"><i class="fas fa-trash-alt"></i></a>';
			$html .= '</div></div>';
		}
		echo '' . $html;
		die();
	...
	function cl_ajax_add_to_cart() {
		...
		if ( isset( $_POST['timestamp'] ) && isset( $_POST['token'] ) && \Clasify\Classified\Front\Purchase\Tokenizer\Tokenizer::is_token_valid( $_POST['token'], $_POST['timestamp'] ) ) {
		...
		foreach ( $to_add as $options ) {
			...
			$key  = CCP()->front->cart->add( $l_id, $options );
	...
	function cl_process_purchase_form() {
		...
		if ( ! CCP()->front->cart->get_contents() && ! CCP()->front->fees->has_fees() ) {

			$valid_data = false;
			CCP()->front->error->cl_set_error( 'empty_cart', __( 'Your cart is empty...', 'clasify-classified-listing' ) );
		} else {
		...
		if ( isset( $_POST['cl_login_submit'] ) ) {
			CCP()->front->gateways->cl_process_purchase_login();
		}

		// Validate the user
		$user = CCP()->front->gateways->cl_get_purchase_form_user( $valid_data );

		// Let extensions validate fields after user is logged in if user has used login/registration form
		do_action( 'cl_checkout_user_error_checks', $user, $valid_data, cl_sanitization( $_POST ) );

		if ( false === $valid_data || CCP()->front->error->cl_get_errors() || ! $user ) {
			if ( $is_ajax ) {
		...
		$existing_payment = CCP()->front->session->get( 'cl_resume_payment' );
		...
		$purchase_data = array(
			'listing'      => CCP()->front->cart->cl_get_cart_contents(),
			'fees'         => CCP()->front->cart->cl_cart_has_fees(),        // Any arbitrary fees that have been added to the cart
			'subtotal'     => CCP()->front->cart->cl_get_cart_subtotal(),    // Amount before taxes and discounts
			'discount'     => CCP()->front->discountaction->cl_get_cart_discounted_amount(), // Discounted amount
			'tax'          => CCP()->front->cart->cl_get_cart_tax(),               // Taxed amount
			'tax_rate'     => CCP()->front->cart->cl_get_cart_tax_rate( $card_country, $card_state, $card_zip ), // Tax rate
			'price'        => CCP()->front->cart->cl_get_cart_total(),    // Amount after taxes
			'purchase_key' => $purchase_key,
			'user_email'   => $user['user_email'],
			'date'         => date( 'Y-m-d H:i:s', current_time( 'timestamp' ) ),
			'user_info'    => stripslashes_deep( $user_info ),
			'post_data'    => cl_sanitization( $_POST ),
			'cart_details' => CCP()->front->cart->cl_get_cart_content_details(),
		...
		CCP()->front->cart->cl_set_purchase_session( $session_data );

		// Send info to the gateway for payment processing
		CCP()->front->gateways->cl_send_to_gateway( $purchase_data['gateway'], $purchase_data );
