...
function cl_resend_purchase_receipt( $data ) {
	...
	if ( ! current_user_can( 'edit_shop_payments' ) ) {
		wp_die( __( 'You do not have permission to edit this payment record', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}
	...
	if ( empty( $email ) ) {
		wp_die( __( 'Email Empty', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}
	...
	if ( is_array( $listing ) ) {
		foreach ( $listing as $listing ) {
			$limit = CCP()->front->listingsaction->cl_get_file_listing_limit( $listing['id'] );
			if ( ! empty( $limit ) ) {
