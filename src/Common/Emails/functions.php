...
function cl_email_purchase_receipt( $payment_id, $admin_notice = true, $to_email = '', $payment = null, $customer = null ) {
	...
	$subject = cl_admin_get_option( 'purchase_subject', __( 'Purchase Receipt', 'clasify-classified-listing' ) );
	$subject = apply_filters( 'cl_purchase_subject', wp_strip_all_tags( $subject ), $payment_id );
	$subject = wp_specialchars_decode( CCP()->common->emailtags->cl_do_email_tags( $subject, $payment_id ) );

	$heading = cl_admin_get_option( 'purchase_heading', __( 'Purchase Receipt', 'clasify-classified-listing' ) );
	$heading = apply_filters( 'cl_purchase_heading', $heading, $payment_id, $payment_data );
	$heading = CCP()->common->emailtags->cl_do_email_tags( $heading, $payment_id );

	$attachments = apply_filters( 'cl_receipt_attachments', array(), $payment_id, $payment_data );

	$message = CCP()->common->emailtags->cl_do_email_tags( CCP()->common->emailtemplate->cl_get_email_body_content( $payment_id, $payment_data ), $payment_id );
...
function cl_email_test_purchase_receipt() {
	...
	$subject = wp_specialchars_decode( CCP()->common->emailtags->cl_do_email_tags( $subject, 0 ) );
	...
	$message = CCP()->common->emailtags->cl_do_email_tags( CCP()->common->emailtemplate->cl_get_email_body_content( 0, array() ), 0 );
...
function cl_admin_email_notice( $payment_id = 0, $payment_data = array() ) {
	...
	$subject = cl_admin_get_option( 'sale_notification_subject', sprintf( __( 'New listing purchase - Order #%1$s', 'clasify-classified-listing' ), $payment_id ) );
	$subject = apply_filters( 'cl_admin_sale_notification_subject', wp_strip_all_tags( $subject ), $payment_id );
	$subject = wp_specialchars_decode( CCP()->common->emailtags->cl_do_email_tags( $subject, $payment_id ) );

	$heading = cl_admin_get_option( 'sale_notification_heading', __( 'New Sale!', 'clasify-classified-listing' ) );
	...
	$message = CCP()->common->emailtemplate->cl_get_sale_notification_body_content( $payment_id, $payment_data );
 ...
 * changed in the CLASIFY CLASSIFIED PLUGIN Settings)
 *
 * @since 1.0
 * @return mixed
 */
...
function cl_get_default_sale_notification_email() {
	...
	$default_email_body .= __( 'Purchased by: ', 'clasify-classified-listing' ) . ' {name}' . "\n";
	$default_email_body .= __( 'Amount: ', 'clasify-classified-listing' ) . ' {price}' . "\n";
	$default_email_body .= __( 'Payment Method: ', 'clasify-classified-listing' ) . ' {payment_method}' . "\n\n";
...
function cl_sendwp_remote_install_handler() {
	if ( ! current_user_can( 'manage_shop_settings' ) ) {
		wp_send_json_error(
			array(
				'error' => __( 'You do not have permission to do this.', 'clasify-classified-listing' ),
			)
		);
	}
	...
	*/
	if ( ! function_exists( 'sendwp_get_server_url' ) ) {
		wp_send_json_error(
			array(
				'error' => __( 'Something went wrong. SendWP was not installed correctly.', 'clasify-classified-listing' ),
			)
		);
	}
...
function cl_recapture_remote_install_handler() {
	...
	*/
	if ( is_wp_error( $activated ) ) {
		wp_send_json_error(
			array(
				'error' => __( 'Something went wrong. Recapture for CLASIFY CLASSIFIED PLUGIN was not installed correctly.', 'clasify-classified-listing' ),
			)
		);
	}
			...
			<p>
				...
				echo wp_kses_post(
					sprintf(
						...
						__( '%1$sRecover abandoned purchases like this one.%2$s %3$sTry Recapture for free%4$s.', 'clasify-classified-listing' ),
						'<strong>',
						'</strong>',
