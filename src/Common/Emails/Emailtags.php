...
class Emailtags {
	...
	function cl_email_tag_exists( $tag ) {
		return CCP()->common->emailtags->email_tag_exists( $tag );
	}
	...
	function cl_do_email_tags( $content, $payment_id ) {
		...
		// Replace all tags
		$content = CCP()->common->emailtags->do_tags( $content, $payment_id );
	...
	function cl_setup_email_tags() {
		...
		$email_tags = array(
			array(
				'tag'         => 'listing_list',
				'description' => __( 'A list of listing links for each listing purchased', 'clasify-classified-listing' ),
				'function'    => 'text/html' == CCP()->common->emails->get_content_type() ? 'cl_email_tag_listing_list' : 'cl_email_tag_listing_list_plain',
			),
			array(
				'tag'         => 'file_urls',
				'description' => __( 'A plain-text list of listing URLs for each listing purchased', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_file_urls',
			),
			array(
				'tag'         => 'name',
				'description' => __( "The buyer's first name", 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_first_name',
			),
			array(
				'tag'         => 'fullname',
				'description' => __( "The buyer's full name, first and last", 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_fullname',
			),
			array(
				'tag'         => 'username',
				'description' => __( "The buyer's user name on the site, if they registered an account", 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_username',
			),
			array(
				'tag'         => 'user_email',
				'description' => __( "The buyer's email address", 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_user_email',
			),
			array(
				'tag'         => 'billing_address',
				'description' => __( 'The buyer\'s billing address', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_billing_address',
			),
			array(
				'tag'         => 'date',
				'description' => __( 'The date of the purchase', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_date',
			),
			array(
				'tag'         => 'subtotal',
				'description' => __( 'The price of the purchase before taxes', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_subtotal',
			),
			array(
				'tag'         => 'tax',
				'description' => __( 'The taxed amount of the purchase', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_tax',
			),
			array(
				'tag'         => 'price',
				'description' => __( 'The total price of the purchase', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_price',
			),
			array(
				'tag'         => 'payment_id',
				'description' => __( 'The unique ID number for this purchase', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_payment_id',
			),
			array(
				'tag'         => 'receipt_id',
				'description' => __( 'The unique ID number for this purchase receipt', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_receipt_id',
			),
			array(
				'tag'         => 'payment_method',
				'description' => __( 'The method of payment used for this purchase', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_payment_method',
			),
			array(
				'tag'         => 'sitename',
				'description' => __( 'Your site name', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_sitename',
			),
			array(
				'tag'         => 'receipt_link',
				'description' => __( 'Adds a link so users can view their receipt directly on your website if they are unable to view it in the browser correctly.', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_receipt_link',
			),
			array(
				'tag'         => 'discount_codes',
				'description' => __( 'Adds a list of any discount codes applied to this purchase', 'clasify-classified-listing' ),
				'function'    => 'cl_email_tag_discount_codes',
			),
			array(
				'tag'         => 'ip_address',
				'description' => __( 'The buyer\'s IP Address', 'clasify-classified-listing' ),
	...
	function cl_email_tag_listing_list( $payment_id ) {
		...
		if ( $cart_items ) {
			...
			foreach ( $cart_items as $item ) {
				...
				if ( CCP()->common->options->cl_use_skus() ) {
					$sku = CCP()->front->listingsaction->cl_get_listing_sku( $item['id'] );
				}
				...
				if ( $show_names ) {
					...
					if ( ! empty( $quantity ) && $quantity > 1 ) {
						$title .= '&nbsp;&ndash;&nbsp;' . __( 'Quantity', 'clasify-classified-listing' ) . ': ' . esc_html( $quantity );
					}

					if ( ! empty( $sku ) ) {
						$title .= '&nbsp;&ndash;&nbsp;' . __( 'SKU', 'clasify-classified-listing' ) . ': ' . esc_html( $sku );
					}

					if ( ! empty( $price_id ) && 0 !== $price_id ) {
						$title .= '&nbsp;&ndash;&nbsp;' . CCP()->front->listingsaction->cl_get_price_option_name( $item['id'], $price_id, $payment_id );
					}

					$listing_list .= '<li>' . apply_filters( 'cl_email_receipt_listing_title', $title, $item, $price_id, $payment_id ) . '<br/>';
				}

				$files = CCP()->front->listingsaction->cl_get_listing_files( $item['id'], $price_id );
				...
				if ( ! empty( $files ) ) {
				...
				} elseif ( CCP()->front->listingsaction->cl_is_bundled_product( $item['id'] ) ) {
					...
					foreach ( $bundled_products as $bundle_item ) {
						...
						$listing_files = CCP()->front->listingsaction->cl_get_listing_files( cl_get_bundle_item_id( $bundle_item ), cl_get_bundle_item_price_id( $bundle_item ) );
					...
					$no_listing_message = apply_filters( 'cl_receipt_no_files_found_text', __( 'No listingable files found.', 'clasify-classified-listing' ), $item['id'] );
				...
				if ( ! array_key_exists( $item['id'], $needs_notes ) ) {
					$item_notes = CCP()->front->listingsaction->cl_get_product_notes( $item['id'] );
					if ( $item_notes ) {
		...
		if ( ! empty( $needs_notes ) ) {
			$listing_list .= __( 'Additional information about your purchase:', 'clasify-classified-listing' );
		...
		if ( $cart_items ) {
			...
			foreach ( $cart_items as $item ) {
				...
				if ( $show_names ) {
					...
					if ( ! empty( $quantity ) && $quantity > 1 ) {
						$title .= __( 'Quantity', 'clasify-classified-listing' ) . ': ' . esc_html( $quantity );
					}

					if ( ! empty( $sku ) ) {
						$title .= __( 'SKU', 'clasify-classified-listing' ) . ': ' . esc_html( $sku );
					}

					if ( $price_id !== null ) {
						$title .= CCP()->front->listingsaction->cl_get_price_option_name( $item['id'], $price_id, $payment_id );
					}
				...
				if ( ! empty( $files ) ) {
					...
					foreach ( $bundled_products as $bundle_item ) {
						...
						$files = CCP()->front->listingsaction->cl_get_listing_files( $bundle_item );
				...
				if ( '' != CCP()->front->listingsaction->cl_get_product_notes( $item['id'] ) ) {
					$listing_list .= "\n";
					$listing_list .= CCP()->front->listingsaction->cl_get_product_notes( $item['id'] ) . "\n";
				}
			}
		}
	...
	function cl_email_tag_subtotal( $payment_id ) {
		$payment  = new Clpayment( $payment_id );
		$subtotal = CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $payment->subtotal ), $payment->currency );
	...
	function cl_email_tag_tax( $payment_id ) {
		$payment = new Clpayment( $payment_id );
		$tax     = CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $payment->tax ), $payment->currency );
	...
	function cl_email_tag_price( $payment_id ) {
		$payment = new Clpayment( $payment_id );
		$price   = CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $payment->total ), $payment->currency );
	...
	function cl_email_tag_payment_method( $payment_id ) {
		$payment = new Clpayment( $payment_id );
		return CCP()->front->gateways->cl_get_gateway_checkout_label( $payment->gateway );
	}
	...
	function cl_email_tag_receipt_link( $payment_id ) {
		...
		$formatted   = sprintf( __( '%1$sView it in your browser %2$s', 'clasify-classified-listing' ), '<a href="' . esc_url( $receipt_url ) . '">', '&raquo;</a>' );
