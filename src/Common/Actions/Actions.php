<?php
namespace Clasify\Classified\Common\Actions;
...
class Actions {
	...
	public function cl_enquiry_form() {
		...
		if ( is_admin() ) { ?>
			<div class="widget_cont">
				<span class="heading"><?php echo esc_html__( 'Clasyfied Enquiry Widget', 'clasify-classified-listing' ); ?></span>
				<span class="info"><?php echo esc_html__( 'Click to edit', 'clasify-classified-listing' ); ?></span>
			</div>
			<?php
		} else {
			?>
				...
				<div id="listing-equiry-form" class="sides-widget-body simple-form">
					<form action="#" method="post">
						...
						<div class="form-group">
							<label><?php _e( 'Name', 'clasify-classified-listing' ); ?></label>
							<input type="text" name="name" required class="form-control" placeholder="<?php _e( 'Your Name', 'clasify-classified-listing' ); ?>">
						</div>
						<div class="form-group">
							<label><?php _e( 'Email', 'clasify-classified-listing' ); ?></label>
							<input type="email" name="email" required class="form-control" placeholder="<?php _e( 'Your Email', 'clasify-classified-listing' ); ?>">
						</div>
						<div class="form-group">
							<label><?php _e( 'Phone No.', 'clasify-classified-listing' ); ?></label>
							<input type="text" name="phone" class="form-control" placeholder="<?php _e( '+001-234-5678', 'clasify-classified-listing' ); ?>">
						</div>
						<div class="form-group">
							<label><?php _e( 'Description', 'clasify-classified-listing' ); ?></label>
							<textarea class="form-control" required name="message" placeholder="<?php _e( 'I\'m interested in this property.', 'clasify-classified-listing' ); ?>"></textarea>
						</div>
						<?php wp_nonce_field( 'clasify-classified-enquiry-form', 'clasify_classified_enquiry' ); ?>
						<input type="hidden" name="action" value="cl_enquiry">
						<input type="hidden" name="listing_email" value="<?php echo esc_attr( $to ); ?>">
						<div id="message"></div>
						<button type="submit" class="btn btn-black btn-md rounded full-width"><?php _e( 'Send Message', 'clasify-classified-listing' ); ?></button>
					</form>
				</div>
			</div>
			<?php
	...
	public function add_cl_user_role() {
		$result = add_role(
			'listing_user',
			__( 'Listing User', 'clasify-classified-listing' ),
			array(
	...
	function cl_discount_field() {
		...
		if ( CCP()->front->discountaction->cl_has_active_discounts() && CCP()->front->cart->cl_get_cart_total() ) :
			...
			<fieldset id="cl_discount_code">
				<p id="cl_show_discount" style="display:none;">
					<?php _e( 'Have a discount code?', 'clasify-classified-listing' ); ?> <a href="#" class="cl_discount_link"><?php echo _x( 'Click to enter it', 'Entering a discount code', 'clasify-classified-listing' ); ?></a>
				</p>
				<p id="cl-discount-code-wrap" class="cl-cart-adjustment">
					<label class="cl-label" for="cl-discount">
						<?php _e( 'Discount', 'clasify-classified-listing' ); ?>
					</label>
					<span class="cl-description"><?php _e( 'Enter a coupon code if you have one.', 'clasify-classified-listing' ); ?></span>
					<span class="cl-discount-code-field-wrap">
						<input class="cl-input" type="text" id="cl-discount" name="cl-discount" placeholder="<?php _e( 'Enter discount', 'clasify-classified-listing' ); ?>" />
						<input type="submit" class="cl-apply-discount cl-submit <?php echo esc_attr( $color ) . ' ' . esc_attr( $style ); ?>" value="<?php echo _x( 'Apply', 'Apply discount at checkout', 'clasify-classified-listing' ); ?>" />
					</span>
	...
	function cl_show_payment_icons() {
		if ( CCP()->front->gateways->cl_show_gateways() && did_action( 'cl_payment_mode_top' ) ) {
			return;
		}
	...
	function cl_checkout_meta_tags() {
		...
		echo '<meta name="cl-chosen-gateway" content="' . esc_attr( CCP()->front->gateways->cl_get_chosen_gateway() ) . '"/>' . "\n";
