...
class Dbcustomer extends Db {
	...
	public function customer_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'selected'    => 0,
			'chosen'      => true,
			'placeholder' => __( 'Select a Customer', 'clasify-classified-listing' ),
			'number'      => 30,
			'data'        => array(
				'search-type'        => 'customer',
				'search-placeholder' => __( 'Type to search all Customers', 'clasify-classified-listing' ),
			),
		);
		...
		if ( $customers ) {
			$options[0] = __( 'No customer attached', 'clasify-classified-listing' );
			foreach ( $customers as $customer ) {
				$options[ absint( $customer->id ) ] = esc_html( $customer->name . ' (' . $customer->email . ')' );
			}
		} else {
			$options[0] = __( 'No customers found', 'clasify-classified-listing' );
		}
		...
		$output = CCP()->admin->settings_instances->select(
			array(
				'name'             => $args['name'],
