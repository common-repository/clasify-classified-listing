<?php
namespace Clasify\Classified\Front\Purchase\Gateways\Stripe;

use Clasify\Classified\Front\Purchase\Gateways\Stripe\Classes\CL_Stripe_Gateway_Exception;
use Clasify\Classified\Front\Purchase\Gateways\Stripe\Classes\CL_Stripe_Functions;
use Clasify\Classified\Front\Purchase\Gateways\Stripe\Classes\CL_Stripe_Rate_Limiting;
...
class Stripe {
	...
	public static function get_setting() {
		...
		return array(
			'stripe_active'        => array(
				'id'   => 'stripe_active',
				'name' => __( 'Stripe Active', 'clasify-classified-listing' ),
				'type' => 'checkbox',
			),
	...
	function cl_process_stripe_purchase( $purchase_data ) {
		...
		if ( isset( $_REQUEST['cl_action'] ) && 'straight_to_gateway' === $_REQUEST['cl_action'] ) {
			...
			CCP()->front->error->cl_unset_error( 'cl-straight-to-gateway-error' );
			CCP()->front->checkout->cl_send_back_to_checkout();
		...
		try {
			...
			if ( has_filter( 'cls_create_charge_args' ) ) {
				...
				foreach ( $compatible_keys as $compatible_key ) {
					...
					cl_debug_log( __( 'Charges are no longer directly created in Stripe.', 'clasify-classified-listing' ), true );
				}
			}
			...
			// Record error in log.
			CCP()->front->gateways->cl_record_gateway_error(
				esc_html__( 'Stripe Error', 'clasify-classified-listing' ),
				sprintf(
					esc_html__( 'There was an error while processing a Stripe payment. Payment data: %s', 'clasify-classified-listing' ),
			...
			if ( isset( $_POST['cl_email'] ) && ! isset( $_POST['payment_method_id'] ) ) {
				...
				CCP()->front->checkout->cl_send_back_to_checkout( '?payment-mode=' . $purchase_data['gateway'] );
			}
