	...
	if ( ! cl_has_upgrade_completed( 'stripe_customer_id_migration' ) ) {
		...
		printf(
			'<div class="updated">' .
				...
				__( 'Clasify Classified Plugin - Stripe Gateway needs to upgrade the customers database; <a href="%s">click here to start the upgrade</a>. <a href="#" onClick="jQuery(this).parent().next(\'p\').slideToggle()">Learn more about this upgrade</a>', 'clasify-classified-listing' ) .
				'</p>' .
				'<p style="display: none;">' .
				__( '<strong>About this upgrade:</strong><br />This upgrade will improve the reliability of associating purchase records with your existing customer records in Stripe by changing their Stripe Customer IDs to be stored locally on their Plugin customer record, instead of their user record.', 'clasify-classified-listing' ) .
				'<br /><br />' .
				__( '<strong>Advanced User?</strong><br />This upgrade can also be run via WPCLI with the following command:<br /><code>wp cl-stripe migrate_customer_ids</code>', 'clasify-classified-listing' ) .
				'</p>' .
				'</div>',
...
function cl_stripe_customer_id_migration() {
	...
	if ( ! current_user_can( 'manage_shop_settings' ) ) {
		wp_die( __( 'You do not have permission to do shop upgrades', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}
