...
function cls_process_preapproved_cancel() {
	...
	cl_insert_payment_note( $payment_id, __( 'Preapproval cancelled', 'clasify-classified-listing' ) );
...
function cls_admin_messages() {
	if ( isset( $_GET['cl-message'] ) && 'preapproval-charged' == $_GET['cl-message'] ) {
		add_settings_error( 'cls-notices', 'cls-preapproval-charged', __( 'The preapproved payment was successfully charged.', 'clasify-classified-listing' ), 'updated' );
	}
	if ( isset( $_GET['cl-message'] ) && 'preapproval-failed' == $_GET['cl-message'] ) {
		add_settings_error( 'cls-notices', 'cls-preapproval-charged', __( 'The preapproved payment failed to be charged. View order details for further details.', 'clasify-classified-listing' ), 'error' );
	}
	if ( isset( $_GET['cl-message'] ) && 'preapproval-cancelled' == $_GET['cl-message'] ) {
		add_settings_error( 'cls-notices', 'cls-preapproval-cancelled', __( 'The preapproved payment was successfully cancelled.', 'clasify-classified-listing' ), 'updated' );
	}

	if ( isset( $_GET['cl_gateway_connect_error'], $_GET['cl-message'] ) ) {
		/* translators: %1$s Stripe Connect error message. %2$s Retry URL. */
		echo '<div class="notice notice-error"><p>' . sprintf( __( 'There was an error connecting your Stripe account. Message: %1$s. Please <a href="%2$s">try again</a>.', 'clasify-classified-listing' ), urldecode( cl_sanitization( $_GET['cl-message'] ) ), esc_url( admin_url( 'edit.php?post_type=listing&page=cl-settings&tab=gateways&section=cl-stripe' ) ) ) . '</p></div>';
		add_filter(
			'wp_parse_str',
			function ( $ar ) {
...
function cls_show_existing_card_meta( $payment_id ) {
	...
	if ( ! empty( $existing_card ) ) {
		...
		<div class="cl-order-stripe-existing-card cl-admin-box-inside">
			<p>
				<span class="label"><?php _e( 'Used Existing Card:', 'clasify-classified-listing' ); ?></span>&nbsp;
				<span><?php _e( 'Yes', 'clasify-classified-listing' ); ?></span>
			</p>
		</div>
		<?php
	}
	...
	<div class="cl-form-group cl-stripe-refund-transaction">
		<div class="cl-form-group__control">
			...
			<label for="cl-stripe-refund" class="cl-form-group__label">
				<?php esc_html_e( 'Refund Charge in Stripe', 'clasify-classified-listing' ); ?>
			</label>
		</div>
	</div>
	<?php
