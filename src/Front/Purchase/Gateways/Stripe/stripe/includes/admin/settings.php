...
function cls_settings_section( $sections ) {
	$sections['cl-stripe'] = __( 'Stripe', 'clasify-classified-listing' );
...
function cls_add_settings( $settings ) {
			...
			array(
				'cl-stripe' => array(
					'cls-requirements-not-met' => array(
						...
						'name'  => __( 'Unmet Requirements', 'clasify-classified-listing' ),
	...
	$stripe_settings = array(
		'stripe_connect_button'       => array(
			...
			'name'  => __( 'Connection Status', 'clasify-classified-listing' ),
		...
		'test_publishable_key'        => array(
			...
			'name'  => __( 'Test Publishable Key', 'clasify-classified-listing' ),
			'desc'  => __( 'Enter your test publishable key, found in your Stripe Account Settings', 'clasify-classified-listing' ),
			'type'  => 'text',
			'size'  => 'regular',
		...
		'test_secret_key'             => array(
			'id'    => 'test_secret_key',
			'name'  => __( 'Test Secret Key', 'clasify-classified-listing' ),
			'desc'  => __( 'Enter your test secret key, found in your Stripe Account Settings', 'clasify-classified-listing' ),
			'type'  => 'text',
			'size'  => 'regular',
			'class' => 'cl-hidden cls-api-key-row',
		),
		'live_publishable_key'        => array(
			...
			'name'  => __( 'Live Publishable Key', 'clasify-classified-listing' ),
			'desc'  => __( 'Enter your live publishable key, found in your Stripe Account Settings', 'clasify-classified-listing' ),
			'type'  => 'text',
			'size'  => 'regular',
			'class' => 'cl-hidden cls-api-key-row',
		),
		'live_secret_key'             => array(
			'id'    => 'live_secret_key',
			'name'  => __( 'Live Secret Key', 'clasify-classified-listing' ),
			'desc'  => __( 'Enter your live secret key, found in your Stripe Account Settings', 'clasify-classified-listing' ),
			'type'  => 'text',
			'size'  => 'regular',
			'class' => 'cl-hidden cls-api-key-row',
		),
		'stripe_webhook_description'  => array(
			...
			'type' => 'descriptive_text',
			'name' => __( 'Webhooks', 'clasify-classified-listing' ),
			'desc' =>
			'<p>' . sprintf(
				/* translators: %1$s Opening anchor tag, do not translate. %2$s Closing anchor tag, do not translate. */
				__( 'In order for Stripe to function completely, you must configure your Stripe webhooks. Visit your %1$saccount dashboard%2$s to configure them. Please add a webhook endpoint for the URL below.', 'clasify-classified-listing' ),
				...
				'<p><strong>' . sprintf(
					...
					__( 'Webhook URL: %s', 'clasify-classified-listing' ),
					home_url( 'index.php?cl-listener=stripe' )
				) . '</strong></p>' .
				'<p>' . sprintf(
					/* translators: %1$s Opening anchor tag, do not translate. %2$s Closing anchor tag, do not translate. */
					__( 'See our %1$sdocumentation%2$s for more information.', 'clasify-classified-listing' ),
		...
		'stripe_billing_fields'       => array(
			...
			'name'    => __( 'Billing Address Display', 'clasify-classified-listing' ),
			'desc'    => __( 'Select how you would like to display the billing address fields on the checkout form. <p><strong>Notes</strong>:</p><p>If taxes are enabled, this option cannot be changed from "Full address".</p><p>If set to "No address fields", you <strong>must</strong> disable "zip code verification" in your Stripe account.</p>', 'clasify-classified-listing' ),
			'type'    => 'select',
			'options' => array(
				'full'        => __( 'Full address', 'clasify-classified-listing' ),
				'zip_country' => __( 'Zip / Postal Code and Country only', 'clasify-classified-listing' ),
				'none'        => __( 'No address fields', 'clasify-classified-listing' ),
			),
			'std'     => 'full',
		),
		'stripe_statement_descriptor' => array(
			'id'   => 'stripe_statement_descriptor',
			'name' => __( 'Statement Descriptor', 'clasify-classified-listing' ),
			'desc' => __( 'Choose how charges will appear on customer\'s credit card statements. <em>Max 22 characters</em>', 'clasify-classified-listing' ),
			'type' => 'text',
		),
		'stripe_use_existing_cards'   => array(
			'id'   => 'stripe_use_existing_cards',
			'name' => __( 'Show Previously Used Cards', 'clasify-classified-listing' ),
			'desc' => __( 'Provides logged in customers with a list of previous used payment methods for faster checkout.', 'clasify-classified-listing' ),
			'type' => 'checkbox',
		),
		'stripe_allow_prepaid'        => array(
			'id'   => 'stripe_allow_prepaid',
			'name' => __( 'Prepaid Cards', 'clasify-classified-listing' ),
			'desc' => __( 'Allow prepaid cards as valid payment method.', 'clasify-classified-listing' ),
			'type' => 'checkbox',
		),
		'stripe_split_payment_fields' => array(
			'id'   => 'stripe_split_payment_fields',
			'name' => __( 'Split Credit Card Form', 'clasify-classified-listing' ),
			'desc' => __( 'Use separate card number, expiration, and CVC fields in payment forms.', 'clasify-classified-listing' ),
			'type' => 'checkbox',
		),
		'stripe_restrict_assets'      => array(
			'id'            => 'stripe_restrict_assets',
			'name'          => ( __( 'Restrict Stripe Assets', 'clasify-classified-listing' ) ),
			'desc'          => ( __( 'Only load Stripe.com hosted assets on pages that specifically utilize Stripe functionality.', 'clasify-classified-listing' ) ),
			'type'          => 'checkbox',
			'tooltip_title' => __( 'Loading Javascript from Stripe', 'clasify-classified-listing' ),
			'tooltip_desc'  => __( 'Stripe advises that their Javascript library be loaded on every page to take advantage of their advanced fraud detection rules. If you are not concerned with this, enable this setting to only load the Javascript when necessary. Read more about Stripe\'s recommended setup here: https://stripe.com/docs/web/setup.', 'clasify-classified-listing' ),
		),
	);
	...
	if ( version_compare( VERSION, 2.5, '>=' ) ) {
		...
		$notice = array(
			'stripe_connect_test_mode_toggle_notice' => array(
				...
				'desc'        => '<p>' . __( 'You have disabled the "Test Mode" option. Once you have saved your changes, please verify your Stripe connection, especially if you have not previously connected in with "Test Mode" disabled.', 'clasify-classified-listing' ) . '</p>',
...
function cl_stripe_sanitize_stripe_billing_fields_save( $value, $key ) {
	...
	if ( 'stripe_billing_fields' == $key && CCP()->front->tax->cl_use_taxes() ) {
...
function cl_stripe_connect_notice_callback( $args ) {
	...
	$html = '<div class="' . esc_attr( $class ) . '" id="cl_settings[' . CCP()->common->formatting->cl_sanitize_key( $args['id'] ) . ']">' . esc_html( $value ) . '</div>';
...
function cl_stripe_checkout_notice_callback( $args ) {
	...
	$html = '<div class="notice notice-warning inline' . cl_sanitize_html_class( $args['field_class'] ) . '" id="cl_settings[' . CCP()->common->formatting->cl_sanitize_key( $args['id'] ) . ']">' . wpautop( $value ) . '</div>';
...
function cl_stripe_requirements_not_met_callback() {
	...
	echo '<strong>';
	esc_html_e( 'Need help upgrading? Ask your web host!', 'clasify-classified-listing' );
	echo '</strong><br />';
...
function cls_payment_gateways_notice( $html, $args ) {
	...
	$html .= '<div id="cls-payment-gateways-stripe-unmet-requirements" class="notice inline notice-info"><p>' .
		...
		'</p><p><strong>' .
		esc_html__( 'Need help upgrading? Ask your web host!', 'clasify-classified-listing' ) .
		'</strong><br />' .
		wp_kses(
			sprintf(
				/* translators: %1$s Opening anchor tag, do not translate. %2$s Closing anchor tag, do not translate. */
