...
function cls_payments_column_data( $value, $payment_id, $column_name ) {
	...
	$actions[] = '<a href="' . esc_url( add_query_arg( $preapproval_args, admin_url( 'edit.php?post_type=listing&page=cl-payment-history' ) ) ) . '">' . __( 'Process', 'clasify-classified-listing' ) . '</a>';

	if ( 'cancelled' !== $status ) {
		$actions[] = '<span class="delete"><a href="' . esc_url( add_query_arg( $cancel_args, admin_url( 'edit.php?post_type=listing&page=cl-payment-history' ) ) ) . '">' . __( 'Cancel', 'clasify-classified-listing' ) . '</a></span>';
	}
