...
function cls_process_gateway_connect_completion() {
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		$message = '<p>' . sprintf(
			...
			__( 'There was an error getting your Stripe credentials. Please %1$stry again%2$s. If you continue to have this problem, please contact support.', 'clasify-classified-listing' ),
...
function cls_stripe_connect_setting_field() {
	...
	<?php if ( empty( $api_key ) ) : ?>
		...
		<a href="<?php echo esc_url( $stripe_connect_url ); ?>" class="cl-stripe-connect">
			<span><?php esc_html_e( 'Connect with Stripe', 'clasify-classified-listing' ); ?></span>
		</a>
		...
		<p>
			...
			echo wp_kses(
				sprintf(
					/* translators: %1$s Opening anchor tag, do not translate. %2$s Closing anchor tag, do not translate. */
					__( 'Have questions about connecting with Stripe? See the %1$sdocumentation%2$s.', 'clasify-classified-listing' ),
	...
	<?php if ( ! empty( $api_key ) ) : ?>
		...
		<div id="cls-stripe-connect-account" class="cls-stripe-connect-acount-info notice inline" data-account-id="<?php echo esc_attr( $stripe_connect_account_id ); ?>" data-nonce="<?php echo wp_create_nonce( 'cls-stripe-connect-account-information' ); ?>">
			<p><span class="spinner is-active"></span>
				<em><?php esc_html_e( 'Retrieving account information...', 'clasify-classified-listing' ); ?></em>
		</div>
		...
		<div class="cls-api-key-toggle">
			<p>
				<button type="button" class="button-link">
					<small>
						<?php esc_html_e( 'Manage API keys manually', 'clasify-classified-listing' ); ?>
					</small>
				</button>
			</p>
		</div>
...
function cls_stripe_connect_account_info_ajax_response() {
	// Generic error.
	$unknown_error = array(
		'message' => wpautop( esc_html__( 'Unable to retrieve account information.', 'clasify-classified-listing' ) ),
	);
	...
	$mode = cl_is_test_mode()
		? _x( 'test', 'Stripe Connect mode', 'clasify-classified-listing' )
		: _x( 'live', 'Stripe Connect mode', 'clasify-classified-listing' );
	...
	$reconnect_disconnect_actions = wp_kses(
		sprintf(
			...
			__( 'Your Stripe account is connected in %1$s mode. %2$sReconnect in %1$s mode%4$s, or %3$sdisconnect this account%4$s.', 'clasify-classified-listing' ),
			'<strong>' . $mode . '</strong>',
			...
			wpautop(
				sprintf(
					...
					sprintf(
						...
						__( '%1$sDisconnect this account%2$s.', 'clasify-classified-listing' ),
						'<a href="' . esc_url( cls_stripe_connect_disconnect_url() ) . '">',
						'</a>'
					)
			),
	...
	if ( ! empty( $account_id ) ) {
		try {
			...
			return wp_send_json_success(
				array(
					'message' => wpautop(
						...
						$display_name . esc_html( $email ) . esc_html__( 'Administrator (Owner)', 'clasify-classified-listing' ) . $fee_message
					),
			...
			return wp_send_json_error(
				array(
					'message' => wpautop(
						esc_html__( 'The API keys provided do not match the Stripe Connect account associated with this installation. If you have manually modified these values after connecting your account, please reconnect below or update your API keys.', 'clasify-classified-listing' ) .
							'<br /><br />' .
		...
		$connect_button = sprintf(
			...
			esc_html__( 'Connect with Stripe', 'clasify-classified-listing' )
		);

		$connect = esc_html__( 'It is highly recommended to Connect with Stripe for easier setup and improved security.', 'clasify-classified-listing' );
				...
				array(
					'message' => wpautop(
						sprintf(
							...
							__( 'Your manually managed %1$s mode API keys are valid.', 'clasify-classified-listing' ),
							'<strong>' . $mode . '</strong>'
						) .
							'<br /><br />' .
			...
			return wp_send_json_error(
				array(
					'message' => wpautop(
						sprintf(
							/* translators: %1$s Stripe payment mode.*/
							__( 'Your manually managed %1$s mode API keys are invalid.', 'clasify-classified-listing' ),
							'<strong>' . $mode . '</strong>'
						) .
							'<br /><br />' .
							$connect . '<br /><br />' . $connect_button
...
function cls_stripe_connect_admin_notices_register() {
	...
	if ( ! $registry ) {
		return new WP_Error( 'cls-invalid-registry', esc_html__( 'Unable to locate registry', 'clasify-classified-listing' ) );
	}
	...
	try {
		...
		$registry->add(
			...
			array(
				'message'     => sprintf(
					'<p>%s</p><p>%s</p>',
					esc_html__( 'Start accepting payments with Stripe by connecting your account. Stripe Connect helps ensure easier setup and improved security.', 'clasify-classified-listing' ),
					$connect_button
				),
				'type'        => 'info',
				'dismissible' => true,
		...
		$test_mode_status = cl_is_test_mode()
			? _x( 'enabled', 'gateway test mode status', 'clasify-classified-listing' )
			: _x( 'disabled', 'gateway test mode status', 'clasify-classified-listing' );
			...
			array(
				'message'     => sprintf(
					...
					sprintf(
						...
						__( '"Test Mode" has been %s. Please verify your Stripe connection status.', 'clasify-classified-listing' ),
						$test_mode_status
					),
					$connect_button
				),
