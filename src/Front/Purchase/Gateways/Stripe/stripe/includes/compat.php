...
function _cls_get_purchase_form_user( $valid_data = array() ) {
	...
	if ( ! empty( $user['user_id'] ) && $user['user_id'] > 0 && ! empty( $user['address'] ) ) {
		if ( function_exists( 'cl_maybe_add_customer_address' ) ) {
			$customer = CCP()->common->dbcustomer->get_customer_by( 'user_id', $user['user_id'] );
			if ( $customer ) {
...
function _cls_process_purchase_form() {
	...
	try {
		...
		if ( ! CCP()->front->cart->cl_get_cart_contents() && ! CCP()->front->cart->cl_cart_has_fees() ) {
			echo '1';
			throw new \Exception( esc_html__( 'Your cart is empty.', 'clasify-classified-listing' ) );
		}

		if ( ! isset( $_POST['cl-process-checkout-nonce'] ) ) {
			echo '2';
			cl_debug_log( __( 'Missing nonce when processing checkout.', 'clasify-classified-listing' ), true );
		}
		...
		if ( false === $nonce_verified ) {
			throw new \Exception( esc_html__( 'Error processing purchase. Please reload the page and try again.', 'clasify-classified-listing' ) );
		}
