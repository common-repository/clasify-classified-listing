...
function cls_credit_card_form( $echo = true ) {
	...
	if ( cl_stripe()->rate_limiting->has_hit_card_error_limit() ) {
		cl_set_error( 'cl_stripe_error_limit', __( 'We are unable to process your payment at this time, please try again later or contact support.', 'clasify-classified-listing' ) );
		return;
	}
	...
	<fieldset id="cl_cc_fields" class="cl-do-validate">
		<legend><?php _e( 'Credit Card Info', 'clasify-classified-listing' ); ?></legend>
		<?php if ( is_ssl() ) : ?>
			<div id="cl_secure_site_wrapper">
				...
				<span><?php _e( 'This is a secure SSL encrypted payment.', 'clasify-classified-listing' ); ?></span>
			</div>
		<?php endif; ?>
...
function cl_stripe_new_card_form() {
	if ( cl_stripe()->rate_limiting->has_hit_card_error_limit() ) {
		cl_set_error( 'cl_stripe_error_limit', __( 'Adding new payment methods is currently unavailable.', 'clasify-classified-listing' ) );
		cl_print_errors();
		return;
	}
	...
	<p id="cl-card-name-wrap">
		<label for="card_name" class="cl-label">
			<?php esc_html_e( 'Name on the Card', 'clasify-classified-listing' ); ?>
			<span class="cl-required-indicator">*</span>
		</label>
		<span class="cl-description"><?php esc_html_e( 'The name printed on the front of your credit card.', 'clasify-classified-listing' ); ?></span>
		<input type="text" name="card_name" id="card_name" class="card-name cl-input required" placeholder="<?php esc_attr_e( 'Card name', 'clasify-classified-listing' ); ?>" autocomplete="cc-name" />
	</p>
	...
	<div id="cl-card-wrap">
		<label for="cl-card-element" class="cl-label">
			<?php
			if ( '1' === $split ) :
				esc_html_e( 'Credit Card Number', 'clasify-classified-listing' );
			else :
				esc_html_e( 'Credit Card', 'clasify-classified-listing' );
			endif;
			?>
			<span class="cl-required-indicator">*</span>
		</label>
		...
		<div id="cl-card-details-wrap">
			...
			<div id="cl-card-exp-wrap">
				<label for="cl-card-exp-element" class="cl-label">
					<?php esc_html_e( 'Expiration', 'clasify-classified-listing' ); ?>
					<span class="cl-required-indicator">*</span>
				</label>
			...
			<div id="cl-card-cvv-wrap">
				<label for="cl-card-exp-element" class="cl-label">
					<?php esc_html_e( 'CVC', 'clasify-classified-listing' ); ?>
					<span class="cl-required-indicator">*</span>
				</label>
...
function cl_stripe_update_billing_address_field() {
	 $payment_mode = strtolower( CCP()->front->gateways->cl_get_chosen_gateway() );
		...
		<label for="cl-stripe-update-billing-address">
			...
			echo wp_kses(
				sprintf(
					...
					__( 'Update card billing address for %1$s •••• %2$s', 'clasify-classified-listing' ),
...
function cl_stripe_existing_card_field_radio( $user_id = 0 ) {
	if ( cl_stripe()->rate_limiting->has_hit_card_error_limit() ) {
		cl_set_error( 'cl_stripe_error_limit', __( 'We are unable to process your payment at this time, please try again later or contacts support.', 'clasify-classified-listing' ) );
		return;
	}
		...
		<div class="cl-stripe-card-selector cl-card-selector-radio">
						...
						<span class="card-label">
							<span class="card-data">
								<span class="card-name-number">
									...
									echo wp_kses(
										sprintf(
											/* translators: %1$s Card type. %2$s Card last 4. */
											__( '%1$s •••• %2$s', 'clasify-classified-listing' ),
							...
							if ( $exp_date < $current ) :
								...
								<span class="card-expired">
									<?php _e( 'Expired', 'clasify-classified-listing' ); ?>
								</span>
								<?php
							endif;
							?>
			...
			<div class="cl-stripe-card-radio-item new-card-wrapper">
				<label for="cl-stripe-add-new">
					...
					<span class="add-new-card"><?php _e( 'Add New Card', 'clasify-classified-listing' ); ?></span>
				</label>
			</div>
		</div>
		<?php
...
function cl_stripe_manage_cards() {
	...
	if ( cl_stripe()->rate_limiting->has_hit_card_error_limit() ) {
		cl_set_error( 'cl_stripe_error_limit', __( 'Payment method management is currently unavailable.', 'clasify-classified-listing' ) );
		cl_print_errors();
		return;
	}
	...
	<div id="cl-stripe-manage-cards">
		<fieldset>
			<legend><?php _e( 'Manage Payment Methods', 'clasify-classified-listing' ); ?></legend>
					...
					<div id="<?php echo esc_attr( $source->id ); ?>_card_item" class="cl-stripe-card-item">
						<span class="card-details">
							...
							<?php if ( $card['default'] ) { ?>
								...
								<span class="card-is-default"><?php _e( 'Default', 'clasify-classified-listing' ); ?></span>
							<?php } ?>
						</span>

						<span class="card-meta">
							<span class="card-expiration"><span class="card-expiration-label"><?php _e( 'Expires', 'clasify-classified-listing' ); ?>: </span><span class="card-expiration-date"><?php echo esc_html( $source->exp_month ); ?>/<?php echo esc_html( $source->exp_year ); ?></span></span>
						...
						<span id="<?php echo esc_attr( $source->id ); ?>-card-actions" class="card-actions">
							<span class="card-update">
								<a href="#" class="cl-stripe-update-card" data-source="<?php echo esc_attr( $source->id ); ?>"><?php _e( 'Update', 'clasify-classified-listing' ); ?></a>
							</span>

							<?php if ( ! $card['default'] ) : ?>
								|
								<span class="card-set-as-default">
									<a href="#" class="cl-stripe-default-card" data-source="<?php echo esc_attr( $source->id ); ?>"><?php _e( 'Set as Default', 'clasify-classified-listing' ); ?></a>
								</span>
								<?php
							endif;
							...
							if ( $can_delete ) :
								...
								<span class="card-delete">
									<a href="#" class="cl-stripe-delete-card delete" data-source="<?php echo esc_attr( $source->id ); ?>"><?php _e( 'Delete', 'clasify-classified-listing' ); ?></a>
								</span>
							<?php endif; ?>
						...
						<form id="<?php echo esc_attr( $source->id ); ?>-update-form" class="card-update-form" data-source="<?php echo esc_attr( $source->id ); ?>">
							...
							<div class="card-address-fields">
								...
								<p class="cls-card-address-field cls-card-address-field--address2">
									...
									echo CCP()->admin->settings_instances->text(
										array(
											...
											'label' => esc_html__( 'Address Line 2', 'clasify-classified-listing' ),
											'name'  => 'address_line2',
								...
								<p class="cls-card-address-field cls-card-address-field--city">
									...
									echo CCP()->admin->settings_instances->text(
										array(
											...
											'label' => esc_html__( 'City', 'clasify-classified-listing' ),
											'name'  => 'address_city',
								...
								<p class="cls-card-address-field cls-card-address-field--zip">
									...
									echo CCP()->admin->settings_instances->text(
										array(
											...
											'label' => esc_html__( 'ZIP Code', 'clasify-classified-listing' ),
											'name'  => 'address_zip',
								...
								<p class="cls-card-address-field cls-card-address-field--country">
									<label for="<?php echo esc_attr( sprintf( 'cls_address_country_%1$s', $source->id ) ); ?>">
										<?php esc_html_e( 'Country', 'clasify-classified-listing' ); ?>
									</label>
									...
									echo CCP()->admin->settings_instances->select(
										array(
											'id'       => sprintf( 'cls_address_country_%1$s', $source->id ),
											'name'     => 'address_country',
											'label'    => esc_html__( 'Country', 'clasify-classified-listing' ),
											'options'  => $countries,
											'selected' => $country,
								...
								<p class="cls-card-address-field cls-card-address-field--state">
									<label for="<?php echo esc_attr( sprintf( 'cls_address_state_%1$s', $source->id ) ); ?>">
										<?php esc_html_e( 'State', 'clasify-classified-listing' ); ?>
									</label>
							...
							<p class="card-expiration-fields">
								<label for="<?php echo esc_attr( sprintf( 'cls_card_exp_month_%1$s', $source->id ) ); ?>" class="cl-label">
									<?php _e( 'Expiration (MM/YY)', 'clasify-classified-listing' ); ?>
								</label>
							...
							<p>
								<input type="submit" class="cl-stripe-submit-update" data-loading="<?php echo esc_attr__( 'Please Wait…', 'clasify-classified-listing' ); ?>" data-submit="<?php echo esc_attr__( 'Update Card', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr__( 'Update Card', 'clasify-classified-listing' ); ?>" />

								<a href="#" class="cl-stripe-cancel-update" data-source="<?php echo esc_attr( $source->id ); ?>"><?php _e( 'Cancel', 'clasify-classified-listing' ); ?></a>
			...
			<form id="cl-stripe-add-new-card">
				<div class="cl-stripe-add-new-card cl-stripe-new-card" style="display: none;">
					<label><?php _e( 'Add New Card', 'clasify-classified-listing' ); ?></label>
					<fieldset id="cl_cc_card_info" class="cc-card-info">
						<legend><?php _e( 'Credit Card Details', 'clasify-classified-listing' ); ?></legend>
						<?php do_action( 'cl_stripe_new_card_form' ); ?>
					</fieldset>
					<?php
					switch ( $display ) {
				...
				<div class="cl-stripe-add-card-actions">
					...
					<input type="submit" class="cl-button cl-stripe-add-new" data-loading="<?php echo esc_attr__( 'Please Wait…', 'clasify-classified-listing' ); ?>" data-submit="<?php echo esc_attr__( 'Add new card', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr__( 'Add new card', 'clasify-classified-listing' ); ?>" />
					<a href="#" id="cl-stripe-add-new-cancel" style="display: none;"><?php _e( 'Cancel', 'clasify-classified-listing' ); ?></a>
...
function cl_stripe_maybe_hide_profile_editor_billing_address() {
	...
	$active_gateways = CCP()->front->gateways->cl_get_enabled_payment_gateways();
...
function cl_stripe_zip_and_country() {
	...
	$customer  = CCP()->front->session->get( 'customer' );
	...
	<fieldset id="cl_cc_address" class="cc-address">
		<legend><?php _e( 'Billing Details', 'clasify-classified-listing' ); ?></legend>
		<p id="cl-card-country-wrap">
			<label for="billing_country" class="cl-label">
				<?php _e( 'Billing Country', 'clasify-classified-listing' ); ?>
				<?php if ( cl_field_is_required( 'billing_country' ) ) { ?>
					<span class="cl-required-indicator">*</span>
				<?php } ?>
			</label>
			<span class="cl-description"><?php _e( 'The country for your billing address.', 'clasify-classified-listing' ); ?></span>
			<select name="billing_country" id="billing_country" class="billing_country cl-select
			<?php
			if ( CCP()->front->checkout->cl_field_is_required( 'billing_country' ) ) {
				echo ' required';
			}
			?>
				" 
		...
		<p id="cl-card-zip-wrap">
			<label for="card_zip" class="cl-label">
				<?php _e( 'Billing Zip / Postal Code', 'clasify-classified-listing' ); ?>
				<?php if ( cl_field_is_required( 'card_zip' ) ) { ?>
					<span class="cl-required-indicator">*</span>
				<?php } ?>
			</label>
			<span class="cl-description"><?php _e( 'The zip or postal code for your billing address.', 'clasify-classified-listing' ); ?></span>
			<input type="text" size="4" name="card_zip" id="card_zip" class="card-zip cl-input
			<?php
			if ( CCP()->front->checkout->cl_field_is_required( 'card_zip' ) ) {
				echo ' required';
			}
			?>
			" placeholder="<?php _e( 'Zip / Postal Code', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr( $customer['address']['zip'] ); ?>" 
			<?php
			if ( CCP()->front->checkout->cl_field_is_required( 'card_zip' ) ) {
				 echo ' required ';
			}
...
function cl_stripe_setup_billing_address_fields() {
	if ( ! method_exists( CCP()->front->tax, 'cl_use_taxes' ) ) {
		return;
	}

	if ( CCP()->front->tax->cl_use_taxes() || 'stripe' !== CCP()->front->gateways->cl_get_chosen_gateway() || ! CCP()->front->cart->cl_get_cart_total() > 0 ) {
		return;
	}
...
function cl_stripe_require_zip_and_country( $fields ) {
	...
	$fields['card_zip'] = array(
		...
		'error_message' => __( 'Please enter your zip / postal code', 'clasify-classified-listing' ),
	);

	$fields['billing_country'] = array(
		'error_id'      => 'invalid_country',
		'error_message' => __( 'Please select your billing country', 'clasify-classified-listing' ),
	);
