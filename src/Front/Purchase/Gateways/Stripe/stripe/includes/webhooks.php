...
function cls_stripe_event_listener() {
	...
	try {
		...
		if ( isset( $event->id ) ) {
			...
			throw new \Exception( esc_html__( 'Unable to find Event', 'clasify-classified-listing' ) );
		}
			...
			case 'charge.refunded':
				...
				if ( $payment && $payment->ID > 0 ) {
					...
					if ( $charge->refunded ) {
						...
						$payment->add_note( sprintf( __( 'Charge %s has been fully refunded in Stripe.', 'clasify-classified-listing' ), $charge->id ) );
						...
						$payment->add_note( sprintf( __( 'Charge %s partially refunded in Stripe.', 'clasify-classified-listing' ), $charge->id ) );
					}
				}
				...
				if ( $payment && $payment->ID > 0 ) {
					$payment->add_note(
						...
						/* translators: %s Stripe Radar review opening reason. */
							__( 'Stripe Radar review opened with a reason of %s.', 'clasify-classified-listing' ),
							$review->reason
						)
					);
					$payment->save();
				...
				if ( $payment && $payment->ID > 0 ) {
					$payment->add_note(
						...
						/* translators: %s Stripe Radar review closing reason. */
							__( 'Stripe Radar review closed with a reason of %s.', 'clasify-classified-listing' ),
							$review->reason
						)
					);
					$payment->save();
