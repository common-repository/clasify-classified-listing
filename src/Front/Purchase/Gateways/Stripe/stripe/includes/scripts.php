...
function cl_stripe_js( $force_load_scripts = false ) {
	...
	if ( method_exists( CCP()->front->checkout, 'cl_is_checkout' ) ) {
		...
		$is_checkout     = CCP()->front->checkout->cl_is_checkout();
		...
		if ( $is_checkout || $force_load_scripts ) {
			...
			$stripe_vars = apply_filters(
				...
				array(
					...
					'submit_text'                    => cl_admin_get_option( 'stripe_checkout_button_text', __( 'Next', 'clasify-classified-listing' ) ),
					...
					'no_key_error'                   => __( 'Stripe publishable key missing. Please enter your publishable key in Settings.', 'clasify-classified-listing' ),
					'checkout_required_fields_error' => __( 'Please fill out all required fields to continue your purchase.', 'clasify-classified-listing' ),
					'checkout_agree_to_terms'        => __( 'Please agree to the terms to complete your purchase.', 'clasify-classified-listing' ),
					'checkout_agree_to_privacy'      => __( 'Please agree to the privacy policy to complete your purchase.', 'clasify-classified-listing' ),
					'generic_error'                  => __( 'Unable to complete your request. Please try again.', 'clasify-classified-listing' ),
					'successPageUri'                 => CCP()->front->checkout->cl_get_success_page_uri(),
					'failurePageUri'                 => CCP()->front->checkout->cl_get_failed_transaction_uri(),
		...
		jQuery(document).ready(function($) {
			$('select[name=cl-payment-status]').change(function() {
				...
				if ('refunded' == $(this).val()) {
					...
					var cl_stripe_refund_charge_label = "<?php echo esc_js( __( 'Refund Charge in Stripe', 'clasify-classified-listing' ) ); ?>";
...
function cl_stripe_connect_admin_script( $hook ) {
	...
	wp_localize_script(
		...
		'cl_stripe_admin',
		array(
			'stripe_enabled'  => array_key_exists( 'stripe', CCP()->front->gateways->cl_get_enabled_payment_gateways() ),
