...
function cls_get_payment_description( $cart_details ) {
	...
	if ( is_array( $cart_details ) && ! empty( $cart_details ) ) {
		foreach ( $cart_details as $item ) {
			...
			if ( false !== $price_id ) {
				$purchase_summary .= ' - ' . CCP()->front->listingsaction->cl_get_price_option_name( $item['id'], $item['item_number']['options']['price_id'] );
			}
...
function cl_stripe_process_refund( $payment_id, $new_status, $old_status ) {
	...
	try {
		...
		wp_die( $e->getMessage(), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 400 ) );
	}
}
	...
	if ( empty( $form_data['cl-stripe-refund'] ) ) {
		...
		cl_add_note(
			array(
				...
				'content'     => __( 'Charge not refunded in Stripe, as checkbox was not selected.', 'clasify-classified-listing' ),
			)
		);
