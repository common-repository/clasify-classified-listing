...
function cls_prb_shim_gateways( $gateways ) {
	...
	$default_gateway_id = CCP()->common->options->cl_get_default_gateway();
	...
	return array_merge(
		array(
			'stripe-prb' => array(
				'admin_label'    => __( 'Express Checkout (Apple Pay/Google Pay)', 'clasify-classified-listing' ),
				'checkout_label' => __( 'Express Checkout', 'clasify-classified-listing' ),
				'supports'       => array(),
			),
		),
		$default_gateway,
...
function cls_prb_load_gateway() {
	if ( ! isset( $_POST['nonce'] ) ) {
		cl_debug_log(
			__( 'Missing nonce when loading the gateway fields. Please read the following for more information.', 'clasify-classified-listing' ),
			true
		);
	}
	...
	<fieldset id="cl_cc_fields" class="cl-do-validate">
		...
		<p id="cl-card-number-wrap">
			<label for="card_number" class="cl-label">
				<?php _e( 'Card Number', 'clasify-classified-listing' ); ?>
				<span class="cl-required-indicator">*</span>
				<span class="card-type"></span>
			</label>
			<span class="cl-description"><?php _e( 'The (typically) 16 digits on the front of your credit card.', 'clasify-classified-listing' ); ?></span>
			<input type="tel" pattern="^[0-9!@#$%^&* ]*$" autocomplete="off" name="card_number" id="card_number" class="card-number cl-input required" placeholder="<?php _e( 'Card number', 'clasify-classified-listing' ); ?>" />
		</p>
		<p id="cl-card-cvc-wrap">
			<label for="card_cvc" class="cl-label">
				<?php _e( 'CVC', 'clasify-classified-listing' ); ?>
				<span class="cl-required-indicator">*</span>
			</label>
			<span class="cl-description"><?php _e( 'The 3 digit (back) or 4 digit (front) value on your card.', 'clasify-classified-listing' ); ?></span>
			<input type="tel" pattern="[0-9]{3,4}" size="4" maxlength="4" autocomplete="off" name="card_cvc" id="card_cvc" class="card-cvc cl-input required" placeholder="<?php _e( 'Security code', 'clasify-classified-listing' ); ?>" />
		</p>
		<p id="cl-card-name-wrap">
			<label for="card_name" class="cl-label">
				<?php _e( 'Name on the Card', 'clasify-classified-listing' ); ?>
				<span class="cl-required-indicator">*</span>
			</label>
			<span class="cl-description"><?php _e( 'The name printed on the front of your credit card.', 'clasify-classified-listing' ); ?></span>
			<input type="text" autocomplete="off" name="card_name" id="card_name" class="card-name cl-input required" placeholder="<?php _e( 'Card name', 'clasify-classified-listing' ); ?>" />
		</p>
		...
		<p class="card-expiration">
			...
			<span class="cl-description"><?php _e( 'The date your credit card expires, typically on the front of the card.', 'clasify-classified-listing' ); ?></span>
