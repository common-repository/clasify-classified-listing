...
function cls_prb_is_enabled( $context = array() ) {
	...
	$taxes = CCP()->front->tax->cl_use_taxes();
...
function cls_prb_get_listing_data( $listing_id, $price_id = false, $quantity = 1 ) {
	$data = array(
		'currency'      => strtolower( CCP()->common->options->cl_get_currency() ),
		'country'       => strtoupper( CCP()->front->country->cl_get_shop_country() ),
		'total'         => array(),
		'display-items' => array(),
	);
	...
	if ( $variable_pricing ) {
		...
		$name = sprintf(
			'%1$s - %2$s',
			$listing->get_name(),
			CCP()->front->listingsaction->cl_get_price_option_name( $listing->ID, $price_id )
		);
	} else {
		$price = $listing->price;
	...
	// Add total.
	$data['total'] = array(
		'label'  => __( 'Total', 'clasify-classified-listing' ),
		'amount' => $price,
	);
	...
	$data['display-items'][] = array(
		'label'  => sprintf(
			'%s%s',
			strip_tags( $name ),
			( $quantity > 1 ? sprintf( __( ' × %d', 'clasify-classified-listing' ), $quantity ) : '' )
		),
		'amount' => $price,
	);
