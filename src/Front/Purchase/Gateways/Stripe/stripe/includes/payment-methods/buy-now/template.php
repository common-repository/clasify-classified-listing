...
function cls_buy_now_modal() {
	...
	echo cls_modal(
		array(
			'id'      => 'cls-buy-now',
			'title'   => __( 'Buy Now', 'clasify-classified-listing' ),
			'class'   => array(
				'cls-buy-now-modal',
			),
...
function cls_buy_now_checkout() {
	$total = (int) CCP()->front->cart->cl_get_cart_total();
	...
	<div id="cl_checkout_form_wrap">
		<form id="cl_purchase_form" class="cl_form cls-buy-now-form" action="<?php echo esc_url( $form_action ); ?>" method="POST">
			...
			<div class="cl_cart_total" style="display: none;">
				<div class="cl_cart_amount" data-total="<?php echo CCP()->front->cart->cl_get_cart_total(); ?>" data-total-currency="<?php echo CCP()->common->formatting->cl_currency_filter( cl_format_amount( CCP()->front->cart->cl_get_cart_total() ) ); ?>">
				</div>
			</div>
...
function cls_buy_now_checkout_purchase_label( $label ) {
	...
	return sprintf(
		'%s - %s',
		CCP()->common->formatting->cl_currency_filter(
...
function cls_buy_now_vars( $vars ) {
	...
	$complete_purchase = ! empty( $label )
		? $label
		: esc_html__( 'Purchase', 'clasify-classified-listing' );
