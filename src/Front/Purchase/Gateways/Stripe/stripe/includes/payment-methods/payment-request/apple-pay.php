...
function cls_prb_apple_pay_admin_notices_register() {
	...
	try {
		...
		$message = ( '<strong>' . esc_html__( 'Apple Pay domain verification error.', 'clasify-classified-listing' ) . '</strong><br />' .
...
function cls_prb_apple_pay_create_directory_and_move_file() {
	...
	if ( ! file_exists( trailingslashit( $fileinfo['path'] ) . $fileinfo['dir'] ) ) {
		if (!@mkdir(trailingslashit($fileinfo['path']) . $fileinfo['dir'], 0755)) { // @codingStandardsIgnoreLine
			throw new \Exception( __( 'Unable to create domain association folder in domain root.', 'clasify-classified-listing' ) );
		}
	}
	...
	if ( ! cls_prb_apple_pay_has_domain_verification_file() ) {
		if (!@copy(trailingslashit(CLS_PLUGIN_DIR) . $fileinfo['file'], $fileinfo['fullpath'])) { // @codingStandardsIgnoreLine
			throw new \Exception( __( 'Unable to copy domain association file to domain .well-known directory.', 'clasify-classified-listing' ) );
		}
	}
}
		...
		if ( empty( $stripe_connect_account_id ) ) {
			...
			throw new \Exception(
				sprintf(
					...
					( __( 'Please %1$smanually add your domain%2$s %3$s to use Apple Pay.', 'clasify-classified-listing' ) . '<br />' ),
