...
function cls_prb_add_settings( $settings ) {
	...
	if ( true === CCP()->front->tax->cl_use_taxes() ) {
		$prb_settings = array(
			array(
				'id'   => 'stripe_prb_taxes',
				'name' => __( 'Apple Pay/Google Pay', 'clasify-classified-listing' ),
			...
			array(
				...
				'desc'    => wp_kses(
					( sprintf(
						...
						__( '"Express Checkout" via Apple Pay, Google Pay, or Microsoft Pay digital wallets. By using Apple Pay, you agree to %1$sStripe%3$s and %2$sApple\'s%3$s terms of service.', 'clasify-classified-listing' ),
						...
						? '<br /><strong>' . __( 'Apple Pay is not available in Test Mode.', 'clasify-classified-listing' ) . '</strong> ' . sprintf(
				...
				'options' => array(
					...
					'single'   => sprintf(
						__( 'Single %s', 'clasify-classified-listing' ),
						cl_get_label_singular()
					),
					/** translators: %s listing noun */
					'archive'  => sprintf(
						__( '%s Archive (includes <code>[listings]</code> shortcode)', 'clasify-classified-listing' ),
						cl_get_label_singular()
					),
					'checkout' => __( 'Checkout', 'clasify-classified-listing' ),
				),
			),
		);
	}
...
function cls_prb_sanitize_setting( $value, $key ) {
	if ( 'stripe_prb' === $key && CCP()->front->tax->cl_use_taxes() ) {
		$value = array();
	}
