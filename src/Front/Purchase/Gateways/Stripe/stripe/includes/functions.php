...
function cls_is_gateway_active() {
	...
	if ( false === CCP()->front->gateways->cl_is_gateway_active( 'stripe' ) ) {
		return false;
	}
...
function cl_refund_stripe_purchase( $order_id_or_object, $refund_object = null ) {
	...
	if ( ! is_numeric( $order_id ) ) {
		throw new \Exception( __( 'Invalid order ID.', 'clasify-classified-listing' ), 400 );
	}
	...
	if ( $refund_object instanceof Order && $order_id_or_object instanceof Order && abs( $refund_object->total ) !== abs( $order_id_or_object->total ) ) {
		...
		cl_debug_log(
			sprintf(
				...
				CCP()->common->formatting->cl_currency_filter( $refund_object->total, $refund_object->currency ),
				$args['amount']
			)
		);
	} else {
	...
	$order_note = sprintf(
		...
		__( '%1$s refunded in Stripe. Refund ID %2$s', 'clasify-classified-listing' ),
		CCP()->common->formatting->cl_currency_filter( $amount_refunded, strtoupper( $refund->currency ) ),
		$refund->id
	);
