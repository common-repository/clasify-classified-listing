...
function cls_preapproved_payment_needs_action_notification( $payment_id ) {
	...
	$subject = esc_html__( 'Your Preapproved Payment Requires Action', 'clasify-classified-listing' );
	$heading = CCP()->common->emailtags->cl_do_email_tags( esc_html__( 'Payment Requires Action', 'clasify-classified-listing' ), $payment_id );

	$message  = esc_html__( 'Dear {name},', 'clasify-classified-listing' ) . "\n\n";
	$message .= esc_html__( 'Your preapproved payment requires further action before your purchase can be completed. Please click the link below to take finalize your purchase', 'clasify-classified-listing' ) . "\n\n";
	$message .= esc_url( add_query_arg( 'payment_key', $payment->key, CCP()->front->checkout->cl_get_success_page_uri() ) );
	$message  = CCP()->common->emailtags->cl_do_email_tags( $message, $payment_id );
