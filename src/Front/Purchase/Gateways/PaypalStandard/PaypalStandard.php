<?php
namespace Clasify\Classified\Front\Purchase\Gateways\PaypalStandard;

use Clasify\Classified\Front\Purchase\Payments\Clpayment;
...
class PaypalStandard {
	...
	function cl_process_paypal_purchase( $purchase_data ) {
		...
		if ( ! $payment ) {
			...
			// Record the error
			CCP()->front->gateways->cl_record_gateway_error( __( 'Payment Error', 'clasify-classified-listing' ), sprintf( __( 'Payment creation failed before sending buyer to PayPal. Payment data: %s', 'clasify-classified-listing' ), json_encode( $payment_data ) ), $payment );
			// Problems? send back
			...
			CCP()->front->session->set( 'cl_resume_payment', $payment );
			...
			$paypal_args = array(
				...
				'shipping'      => '0',
				'no_note'       => '1',
				'currency_code' => CCP()->common->options->cl_get_currency(),
				'charset'       => get_bloginfo( 'charset' ),
				'custom'        => $payment,
				'rm'            => '2',
				'return'        => $return_url,
				'cancel_return' => CCP()->front->checkout->cl_get_failed_transaction_uri( '?payment-id=' . $payment ),
			...
			if ( is_array( $purchase_data['cart_details'] ) && ! empty( $purchase_data['cart_details'] ) ) {
				foreach ( $purchase_data['cart_details'] as $item ) {
					...
					$nameofitem = stripslashes_deep( html_entity_decode( CCP()->front->cart->cl_get_cart_item_name( $item ), ENT_COMPAT, 'UTF-8' ) );

					$paypal_args[ 'item_name_' . $i ] = $nameofitem;
					$paypal_args[ 'quantity_' . $i ]  = $item['quantity'];
					$paypal_args[ 'amount_' . $i ]    = $item_amount;

					if ( CCP()->common->options->cl_use_skus() ) {
						$paypal_args[ 'item_number_' . $i ] = CCP()->front->listingsaction->cl_get_listing_sku( $item['id'] );
					}
			...
			if ( ! empty( $purchase_data['fees'] ) ) {
				...
				foreach ( $purchase_data['fees'] as $fee ) {
					if ( empty( $fee['listing_id'] ) && floatval( $fee['amount'] ) > '0' ) {
						...
						$paypal_args[ 'amount_' . $i ]    = CCP()->common->formatting->cl_sanitize_amount( $fee['amount'] );
						$i++;
			...
			if ( $discounted_amount > '0' ) {
				$paypal_args['discount_amount_cart'] = CCP()->common->formatting->cl_sanitize_amount( $discounted_amount );
			...
			if ( CCP()->front->tax->cl_use_taxes() ) {

				$paypal_args['tax_cart'] = CCP()->common->formatting->cl_sanitize_amount( $purchase_data['tax'] );
			}
	...
	function cl_process_paypal_ipn() {
		...
		if ( is_wp_error( $api_response ) ) {
			CCP()->front->gateways->cl_record_gateway_error( __( 'IPN Error', 'clasify-classified-listing' ), sprintf( __( 'Invalid IPN verification response. IPN data: %s', 'clasify-classified-listing' ), json_encode( $api_response ) ) );
	...
	function cl_process_paypal_web_accept_and_cart( $data, $payment_id ) {
		...
		if ( strcasecmp( $business_email, trim( cl_admin_get_option( 'paypal_email', false ) ) ) != 0 ) {
			CCP()->front->gateways->cl_record_gateway_error( __( 'IPN Error', 'clasify-classified-listing' ), sprintf( __( 'Invalid business email in IPN response. IPN data: %s', 'clasify-classified-listing' ), json_encode( $data ) ), $payment_id );
			cl_debug_log( 'Invalid business email in IPN response. IPN data: ' . print_r( $data, true ) );
			cl_update_payment_status( $payment_id, 'failed' );
			cl_insert_payment_note( $payment_id, __( 'Payment failed due to invalid PayPal business email.', 'clasify-classified-listing' ) );
			return;
		}
		...
		if ( $currency_code != strtolower( $payment->currency ) ) {
			...
			CCP()->front->gateways->cl_record_gateway_error( __( 'IPN Error', 'clasify-classified-listing' ), sprintf( __( 'Invalid currency in IPN response. IPN data: %s', 'clasify-classified-listing' ), json_encode( $data ) ), $payment_id );
			cl_debug_log( 'Invalid currency in IPN response. IPN data: ' . print_r( $data, true ) );
			cl_update_payment_status( $payment_id, 'failed' );
			cl_insert_payment_note( $payment_id, __( 'Payment failed due to invalid currency in PayPal IPN.', 'clasify-classified-listing' ) );
			return;
		}
		...
		if ( $payment_status == 'refunded' || $payment_status == 'reversed' ) {
			...
			if ( number_format( (float) $paypal_amount, 2 ) < number_format( (float) $payment_amount, 2 ) ) {
				...
				CCP()->front->gateways->cl_record_gateway_error( __( 'IPN Error', 'clasify-classified-listing' ), sprintf( __( 'Invalid payment amount in IPN response. IPN data: %s', 'clasify-classified-listing' ), json_encode( $data ) ), $payment_id );
				// cl_debug_log('Invalid payment amount in IPN response. IPN data: ' . printf($data, true));
				cl_update_payment_status( $payment_id, 'failed' );
				cl_insert_payment_note( $payment_id, __( 'Payment failed due to invalid amount in PayPal IPN.', 'clasify-classified-listing' ) );
				return;
			}
			if ( $purchase_key != cl_get_payment_key( $payment_id ) ) {
				...
				CCP()->front->gateways->cl_record_gateway_error( __( 'IPN Error', 'clasify-classified-listing' ), sprintf( __( 'Invalid purchase key in IPN response. IPN data: %s', 'clasify-classified-listing' ), json_encode( $data ) ), $payment_id );
				cl_update_payment_status( $payment_id, 'failed' );
				cl_insert_payment_note( $payment_id, __( 'Payment failed due to invalid purchase key in PayPal IPN.', 'clasify-classified-listing' ) );
				return;
			}

			if ( 'completed' == $payment_status || cl_is_test_mode() ) {

				cl_insert_payment_note( $payment_id, sprintf( __( 'PayPal Transaction ID: %s', 'clasify-classified-listing' ), $data['txn_id'] ) );
				...
				switch ( strtolower( $data['pending_reason'] ) ) {
					...
					case 'echeck':
						$note            = __( 'Payment made via eCheck and will clear automatically in 5-8 days', 'clasify-classified-listing' );
						$payment->status = 'processing';
						$payment->save();
						break;

					case 'address':
						$note = __( 'Payment requires a confirmed customer address and must be accepted manually through PayPal', 'clasify-classified-listing' );

						break;

					case 'intl':
						$note = __( 'Payment must be accepted manually through PayPal due to international account regulations', 'clasify-classified-listing' );

						break;

					case 'multi-currency':
						$note = __( 'Payment received in non-shop currency and must be accepted manually through PayPal', 'clasify-classified-listing' );

						break;

					case 'paymentreview':
					case 'regulatory_review':
						$note = __( 'Payment is being reviewed by PayPal staff as high-risk or in possible violation of government regulations', 'clasify-classified-listing' );

						break;

					case 'unilateral':
						$note = __( 'Payment was sent to non-confirmed or non-registered email address.', 'clasify-classified-listing' );

						break;

					case 'upgrade':
						$note = __( 'PayPal account must be upgraded before this payment can be accepted', 'clasify-classified-listing' );

						break;

					case 'verify':
						$note = __( 'PayPal account is not verified. Verify account in order to accept this payment', 'clasify-classified-listing' );

						break;

					case 'other':
						$note = __( 'Payment is pending for unknown reasons. Contact PayPal support for assistance', 'clasify-classified-listing' );
	...
	function cl_process_paypal_refund( $data, $payment_id = 0 ) {
		...
		if ( number_format( (float) $refund_amount, 2 ) < number_format( (float) $payment_amount, 2 ) ) {
			...
			cl_insert_payment_note( $payment_id, sprintf( __( 'Partial PayPal refund processed: %s', 'clasify-classified-listing' ), $data['parent_txn_id'] ) );
			return; // This is a partial refund

		}

		cl_insert_payment_note( $payment_id, sprintf( __( 'PayPal Payment #%1$s Refunded for reason: %2$s', 'clasify-classified-listing' ), $data['parent_txn_id'], $data['reason_code'] ) );
		cl_insert_payment_note( $payment_id, sprintf( __( 'PayPal Refund Transaction ID: %s', 'clasify-classified-listing' ), $data['txn_id'] ) );
	...
	function cl_paypal_process_pdt_on_return() {
		...
		if ( ! cl_is_success_page() || ! $token || ! CCP()->front->gateways->cl_is_gateway_active( 'paypal' ) ) {
			cl_debug_log( 'return' );
			return;
		}
			...
			if ( ! is_wp_error( $request ) ) {
				...
				if ( strcmp( $lines[0], 'SUCCESS' ) == 0 ) {
					...
					if ( is_null( $total ) ) {
						...
						$payment->add_note( __( 'Payment could not be verified while validating PayPal PDT. Missing payment total fields.', 'clasify-classified-listing' ) );
						...
						$payment->add_note( sprintf( __( 'Payment failed while validating PayPal PDT. Amount expected: %1$f. Amount Received: %2$f', 'clasify-classified-listing' ), $payment->total, $data['payment_gross'] ) );
					...
					$payment->add_note( __( 'Payment failed while validating PayPal PDT.', 'clasify-classified-listing' ) );
					$payment->status = 'failed';
					$payment->save();
				} else {

					// cl_debug_log('Attempt to verify PayPal payment with PDT met with an unexpected result: ' . print_r($body, true));
					$payment->add_note( __( 'PayPal PDT encountered an unexpected result, payment set to pending', 'clasify-classified-listing' ) );
					$payment->status = 'pending';
					$payment->save();
				}
			} else {
	...
	function cl_paypal_refund_admin_js( $payment_id = 0 ) {
		...
		$label = __( 'Refund Payment in PayPal', 'clasify-classified-listing' );
		?>
	...
	function cl_refund_paypal_purchase( $payment ) {
		...
		if ( is_wp_error( $request ) ) {
			...
			if ( isset( $body['ACK'] ) && 'success' === strtolower( $body['ACK'] ) ) {
				...
				if ( isset( $body['L_LONGMESSAGE0'] ) ) {
					...
					$error_msg = __( 'PayPal refund failed for unknown reason.', 'clasify-classified-listing' );
				}
			}
		}
		...
		if ( $success ) {
			...
			$payment->add_note( sprintf( __( 'PayPal refund transaction ID: %s', 'clasify-classified-listing' ), $body['REFUNDTRANSACTIONID'] ) );
		} else {

			$payment->add_note( sprintf( __( 'PayPal refund failed: %s', 'clasify-classified-listing' ), $error_msg ) );
		}
	...
	public static function get_setting() {
		return array(
			...
			'paypal_active'                => array(
				'id'   => 'paypal_active',
				'name' => __( 'Paypal Active', 'clasify-classified-listing' ),
				'type' => 'checkbox',
			),

			'paypal_title'                 => array(
				'id'   => 'paypal_title',
				'name' => __( 'Title', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_des'                   => array(
				'id'   => 'paypal_des',
				'name' => __( 'Description', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_email'                 => array(
				'id'   => 'paypal_email',
				'name' => __( 'Paypal Email', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_sandbox'               => array(
				'id'   => 'paypal_sandbox_active',
				'name' => __( 'Paypal Sandbox', 'clasify-classified-listing' ),
				'type' => 'checkbox',
			),
			'paypal_email_notify_ipn'      => array(
				'id'   => 'paypal_email_notify_ipn',
				'name' => __( 'IPN email notifications', 'clasify-classified-listing' ),
				'type' => 'checkbox',
			),
			'paypal_email_reciver'         => array(
				'id'   => 'paypal_email_reciver',
				'name' => __( 'Receiver email', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_identity_token'        => array(
				'id'   => 'paypal_identity_token',
				'name' => __( 'PayPal identity token', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_invoice_prefix'        => array(
				'id'   => 'paypal_invoice_prefix',
				'name' => __( 'Invoice Prefix', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_image_url'             => array(
				'id'   => 'paypal_image_url',
				'name' => __( 'Image Url', 'clasify-classified-listing' ),
				'type' => 'url',
			),

			'paypal_api_header'            => array(
				'id'   => 'paypal_api_header',
				'name' => __( 'API credentials', 'clasify-classified-listing' ),
				'type' => 'header',
			),

			'paypal_live_api_username'     => array(
				'id'   => 'paypal_live_api_username',
				'name' => __( 'Live API username', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_live_api_password'     => array(
				'id'   => 'paypal_live_api_password',
				'name' => __( 'Live API password', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_live_api_signature'    => array(
				'id'   => 'paypal_live_api_signature',
				'name' => __( 'Live API signature', 'clasify-classified-listing' ),
				'type' => 'text',
			),

			'paypal_sandbox_api_username'  => array(
				'id'   => 'paypal_sandbox_api_username',
				'name' => __( 'Sandbox API username', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_sandbox_api_password'  => array(
				'id'   => 'paypal_sandbox_api_password',
				'name' => __( 'Sandbox API password', 'clasify-classified-listing' ),
				'type' => 'text',
			),
			'paypal_sandbox_api_signature' => array(
				'id'   => 'paypal_sandbox_api_signature',
				'name' => __( 'Sandbox API signature', 'clasify-classified-listing' ),
				'type' => 'text',
			),
