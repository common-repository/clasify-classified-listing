...
class Gateways {
	...
	public function cl_get_chosen_gateway() {
		...
		if ( CCP()->front->cart->cl_get_cart_subtotal() <= 0 ) {
	...
	public function cl_get_payment_gateways() {
		...
		$gateways = array(
			'paypal' => array(
				'admin_label'    => __( 'PayPal Standard', 'clasify-classified-listing' ),
				'checkout_label' => __( 'PayPal', 'clasify-classified-listing' ),
				'supports'       => array( 'buy_now' ),
			),
			'stripe' => array(
				'admin_label'    => __( 'Stripe Payment', 'clasify-classified-listing' ),
				'checkout_label' => __( 'Stripe Payment', 'clasify-classified-listing' ),
			),
		);
	...
	function cl_get_gateway_admin_label( $gateway ) {
		...
		if ( $gateway == 'manual' && $payment ) {
			if ( cl_get_payment_amount( $payment ) == 0 ) {
				$label = __( 'Free Purchase', 'clasify-classified-listing' );
			}
		}
	...
	public function cl_shop_supports_buy_now() {
		...
		if ( ! CCP()->front->tax->cl_use_taxes() && $gateways && 1 === count( $gateways ) ) {
	...
	function cl_purchase_form_validate_gateway() {
		...
		if ( ! empty( $_REQUEST['cl-gateway'] ) ) {
			...
			if ( '0.00' == CCP()->front->cart->cl_get_cart_total() ) {

				$gateway = 'manual';
			} elseif ( ! $this->cl_is_gateway_active( $gateway ) ) {

				CCP()->front->error->cl_set_error( 'invalid_gateway', __( 'The selected payment gateway is not enabled', 'clasify-classified-listing' ) );
			}
		}
	...
	function cl_purchase_form_validate_discounts() {
		...
		if ( ! empty( $_POST['cl-discount'] ) && __( 'Enter discount', 'clasify-classified-listing' ) != $_POST['cl-discount'] ) {
			...
			if ( $posted_discount && ( empty( $discounts ) || CCP()->front->discountaction->cl_multiple_discounts_allowed() ) && CCP()->front->discountaction->cl_is_discount_valid( $posted_discount, $user ) ) {
				CCP()->front->discountaction->cl_set_cart_discount( $posted_discount );
			}
		}
		...
		if ( ! empty( $discounts ) ) {
			foreach ( $discounts as $discount ) {
				// Check if valid
				if ( ! CCP()->front->discountaction->cl_is_discount_valid( $discount, $user ) ) {
		...
		if ( $error ) {
			CCP()->front->error->cl_set_error( 'invalid_discount', __( 'One or more of the discounts you entered is invalid', 'clasify-classified-listing' ) );
		}
	}
	...
	function cl_purchase_form_validate_cc() {
		...
		if ( ! empty( $card_data['card_zip'] ) ) {
			if ( ! $this->cl_purchase_form_validate_cc_zip( $card_data['card_zip'], $card_data['card_country'] ) ) {
				CCP()->front->error->cl_set_error( 'invalid_cc_zip', __( 'The zip / postal code you entered for your billing address is invalid', 'clasify-classified-listing' ) );
			}
		}
	...
	function cl_purchase_form_validate_guest_user() {
		...
		if ( cl_logged_in_only() ) {
			CCP()->front->error->cl_set_error( 'logged_in_only', __( 'You must be logged into an account to purchase', 'clasify-classified-listing' ) );
		}
		...
		if ( $guest_email && strlen( $guest_email ) > 0 ) {
			...
			if ( ! is_email( $guest_email ) ) {
				// Invalid email
				CCP()->front->error->cl_set_error( 'email_invalid', __( 'Invalid email', 'clasify-classified-listing' ) );
			} else {
				// All is good to go
			...
			// No email
			CCP()->front->error->cl_set_error( 'email_empty', __( 'Enter an email', 'clasify-classified-listing' ) );
		}
		...
		foreach ( $this->cl_purchase_form_required_fields() as $field_name => $value ) {
			if ( in_array( $value, $this->cl_purchase_form_required_fields() ) && empty( $_POST[ $field_name ] ) ) {
				CCP()->front->error->cl_set_error( $value['error_id'], $value['error_message'] );
			}
		}
	...
	function cl_check_purchase_email( $valid_data, $posted ) {
		...
		foreach ( $user_emails as $email ) {
			if ( cl_is_email_banned( $email ) ) {
				...
				CCP()->front->error->cl_set_error( 'email_banned', __( 'An internal error has occurred, please try again or contact support.', 'clasify-classified-listing' ) );
				break;
			}
		}
	}
	...
	function cl_process_straight_to_gateway( $data ) {
		...
		if ( ! array_key_exists( $purchase_data['gateway'], $enabled_gateways ) ) {
			...
			CCP()->front->error->cl_set_error( 'cl-straight-to-gateway-error', __( 'There was an error completing your purchase. Please try again.', 'clasify-classified-listing' ) );
	...
	function cl_purchase_form_validate_logged_in_user() {
		...
		if ( $user_ID > 0 ) {
			...
			if ( $user_data ) {
				...
				CCP()->front->error->cl_set_error( 'invalid_user', __( 'The user information is invalid', 'clasify-classified-listing' ) );
			}
		}
	...
	function cl_purchase_form_required_fields() {
		$required_fields = array(
			...
			'cl_first' => array(
				...
				'error_message' => __( 'Please enter your first name', 'clasify-classified-listing' ),
			),
		);

		// Let payment gateways and other extensions determine if address fields should be required
		$require_address = apply_filters( 'cl_require_billing_address', CCP()->front->tax->cl_use_taxes() && CCP()->front->cart->cl_get_cart_total() );
		...
		if ( $require_address ) {
			...
			$required_fields['card_city']       = array(
				...
				'error_message' => __( 'Please enter your billing city', 'clasify-classified-listing' ),
			);
			...
			$required_fields['card_state']      = array(
				...
				'error_message' => __( 'Please enter billing state / province', 'clasify-classified-listing' ),
			);
			...
			if ( isset( $_POST['billing_country'] ) && isset( $required_fields['card_state'] ) ) {
				...
				$states                   = CCP()->front->country->cl_get_shop_states( $customer_billing_country );
	...
	function cl_get_purchase_form_user( $valid_data = array() ) {
			...
			if ( $valid_data['need_new_user'] === true ) {
				...
				*/
				...
				if ( empty( $user ) || $user['user_id'] == -1 ) {
					cl_set_error( 'invalid_user', __( 'The user information is invalid', 'clasify-classified-listing' ) );
					return false;
				} else {
		...
		if ( false === $user && false === CCP()->common->options->cl_no_guest_checkout() ) {
			// Set user
