<?php
namespace Clasify\Classified\Front\Purchase\Discount;
...
class Discount {
	...
	public function __get( $key ) {
		...
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'cl-discount-invalid-property', sprintf( __( 'Can\'t get property %s', 'clasify-classified-listing' ), $key ) );
		}
	}
	...
	private function find_by_code( $code = '' ) {
		...
		$discounts = CCP()->front->discountaction->cl_get_discounts(
			array(
	...
	private function find_by_name( $name = '' ) {
		...
		$discounts = CCP()->admin->discount_action->cl_get_discounts(
			array(
	...
	public function get_status_label() {
		switch ( $this->status ) {
			case 'expired':
				$label = __( 'Expired', 'clasify-classified-listing' );
				break;
			case 'inactive':
				$label = __( 'Inactive', 'clasify-classified-listing' );
				break;
			case 'active':
			default:
				$label = __( 'Active', 'clasify-classified-listing' );
				break;
		}
	...
	public function is_started( $set_error = true ) {
		...
		if ( $this->start ) {
			...
			if ( $start_date < time() ) {
				...
				CCP()->front->error->cl_set_error( 'cl-discount-error', _x( 'This discount is invalid.', 'error shown when attempting to use a discount before its start date', 'clasify-classified-listing' ) );
			}
		} else {
	...
	public function is_maxed_out( $set_error = true ) {
		...
		if ( $this->uses >= $this->max_uses && ! empty( $this->max_uses ) ) {
			if ( $set_error ) {
				CCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount has reached its maximum usage.', 'clasify-classified-listing' ) );
			}
	...
	public function is_min_price_met( $set_error = true ) {
		...
		if ( (float) $cart_amount >= (float) $this->min_price ) {
			...
			CCP()->front->error->cl_set_error( 'cl-discount-error', sprintf( __( 'Minimum order of %s not met.', 'clasify-classified-listing' ), cl_currency_filter( cl_format_amount( $this->min_price ) ) ) );
		}
	...
	public function is_product_requirements_met( $set_error = true ) {
				...
				case 'all':
					...
					foreach ( $product_reqs as $listing_id ) {
						...
						if ( ! cl_item_in_cart( $listing_id ) ) {
							...
							if ( $set_error ) {
								CCP()->front->error->cl_set_error( 'cl-discount-error', __( 'The product requirements for this discount are not met.', 'clasify-classified-listing' ) );
							}
		...
		if ( ! empty( $excluded_ps ) ) {
			if ( count( array_intersect( $cart_ids, $excluded_ps ) ) == count( $cart_ids ) ) {
				...
				if ( $set_error ) {
					CCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount is not valid for the cart contents.', 'clasify-classified-listing' ) );
				}
			}
		}
	...
	public function is_used( $user = '', $set_error = true ) {
		...
		if ( $this->is_single_use ) {
			...
			if ( CCP()->common->dbcustomer->installed() ) {
			...
			if ( $payments ) {
				foreach ( $payments as $payment ) {
					...
					if ( is_array( $discounts ) ) {
						...
						if ( false !== $key ) {
							if ( $set_error ) {
								CCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount has already been redeemed.', 'clasify-classified-listing' ) );
							}
	...
	public function is_valid( $user = '', $set_error = true ) {
		...
		if ( cl_get_cart_contents() && $this->ID ) {
			...
			CCP()->front->error->cl_set_error( 'cl-discount-error', _x( 'This discount is invalid.', 'error for when a discount is invalid based on its configuration', 'clasify-classified-listing' ) );
		}
		...
		if ( $this->exists() ) {
			if ( $this->is_expired( $update ) ) {
				if ( defined( 'DOING_AJAX' ) && $set_error ) {
					CCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount is expired.', 'clasify-classified-listing' ) );
				}
			} elseif ( $this->post_status == 'active' ) {
				$return = true;
			} elseif ( defined( 'DOING_AJAX' ) && $set_error ) {
				CCP()->front->error->cl_set_error( 'cl-discount-error', __( 'This discount is not active.', 'clasify-classified-listing' ) );
			}
		}
