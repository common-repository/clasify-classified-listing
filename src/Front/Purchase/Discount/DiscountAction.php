...
class DiscountAction {
	...
	function cl_format_discount_rate( $type, $amount ) {
		if ( $type == 'flat' ) {
			return CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $amount ) );
		} else {
			return $amount . '%';
		}
	}
	...
	function cl_set_cart_discount( $code = '' ) {
		...
		CCP()->front->session->set( 'cart_discounts', implode( '|', $discounts ) );
	...
	function cl_unset_cart_discount( $code = '' ) {
		...
		if ( $discounts ) {
			...
			CCP()->front->session->set( 'cart_discounts', $discounts );
		}
	...
	function cl_unset_all_cart_discounts() {
		CCP()->front->cart->remove_all_discounts();
	}
	...
	function cl_get_cart_discounts() {
		return CCP()->front->cart->get_discounts();
	}
	...
	function cl_cart_has_discounts() {
		return CCP()->front->cart->has_discounts();
	}
	...
	function cl_get_cart_discounted_amount( $discounts = false ) {
		return CCP()->front->cart->get_discounted_amount( $discounts );
	}
	...
	function cl_get_cart_item_discount_amount( $item = array(), $discount = false ) {
		return CCP()->front->cart->get_item_discount_amount( $item, $discount );
	}
	...
	function cl_get_cart_discounts_html( $discounts = false ) {
		if ( ! $discounts ) {
			$discounts = CCP()->front->cart->get_discounts();
		}
	...
	function cl_display_cart_discount( $formatted = false, $echo = false ) {
		if ( ! $echo ) {
			return CCP()->front->cart->display_cart_discount( $echo );
		} else {
			CCP()->front->cart->display_cart_discount( $echo );
		}
	}
	...
	function cl_listen_for_cart_discount() {
		...
		CCP()->front->session->set( 'preset_discount', $code );
	}
	...
	function cl_apply_preset_discount() {
		$code = cl_sanitization( CCP()->front->session->get( 'preset_discount' ) );
		...
		CCP()->front->session->set( 'preset_discount', null );
	}
