<?php
namespace Clasify\Classified\Front\Purchase\Checkout;
...
class Checkout {
	...
	function cl_user_info_fields() {
		...
		<fieldset id="cl_checkout_user_info">
			<legend><?php echo apply_filters( 'cl_checkout_personal_info_text', esc_html__( 'Personal Info', 'clasify-classified-listing' ) ); ?></legend>
			<?php do_action( 'cl_purchase_form_before_email' ); ?>
			<p id="cl-email-wrap">
				<label class="cl-label" for="cl-email">
					<?php esc_html_e( 'Email Address', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'cl_email' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description" id="cl-email-description"><?php esc_html_e( 'We will send the purchase receipt to this address.', 'clasify-classified-listing' ); ?></span>
				<input class="cl-input required" type="email" name="cl_email" placeholder="<?php esc_html_e( 'Email address', 'clasify-classified-listing' ); ?>" id="cl-email" value="<?php echo esc_attr( $customer['email'] ); ?>" aria-describedby="cl-email-description" 
																											 <?php
			...
			<p id="cl-first-name-wrap">
				<label class="cl-label" for="cl-first">
					<?php esc_html_e( 'First Name', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'cl_first' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description" id="cl-first-description"><?php esc_html_e( 'We will use this to personalize your account experience.', 'clasify-classified-listing' ); ?></span>
				<input class="cl-input required" type="text" name="cl_first" placeholder="<?php esc_html_e( 'First Name', 'clasify-classified-listing' ); ?>" id="cl-first" value="<?php echo esc_attr( $customer['first_name'] ); ?>" 
																											<?php
			...
			<p id="cl-last-name-wrap">
				<label class="cl-label" for="cl-last">
					<?php esc_html_e( 'Last Name', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'cl_last' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description" id="cl-last-description"><?php esc_html_e( 'We will use this as well to personalize your account experience.', 'clasify-classified-listing' ); ?></span>
				...
				?>
										" type="text" name="cl_last" id="cl-last" placeholder="<?php esc_html_e( 'Last Name', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr( $customer['last_name'] ); ?>" 
										<?php
										if ( $this->cl_field_is_required( 'cl_last' ) ) {
																																																					  echo ' required ';
										}
	...
	function cl_checkout_final_total() {
		...
		<p id="cl_final_total_wrap">
			<strong><?php _e( 'Purchase Total:', 'clasify-classified-listing' ); ?></strong>
			<span class="cl_cart_amount" data-subtotal="<?php echo CCP()->front->cart->get_subtotal(); ?>" data-total="<?php echo CCP()->front->cart->get_total(); ?>"><?php CCP()->front->cart->cl_cart_total(); ?></span>
		</p>
		<?php
	}
	...
	function cl_checkout_submit() {
		...
		<fieldset id="cl_purchase_submit">
			...
			<?php if ( cl_is_ajax_disabled() ) { ?>
				<p class="cl-cancel"><a href="<?php echo cl_get_checkout_uri(); ?>"><?php _e( 'Go back', 'clasify-classified-listing' ); ?></a></p>
			<?php } ?>
		</fieldset>
		<?php
	}
	...
	function cl_checkout_hidden_fields() {
		...
		<input type="hidden" name="cl-gateway" value="<?php echo CCP()->front->gateways->cl_get_chosen_gateway(); ?>" />
	...
	function cl_checkout_button_purchase() {
		...
		$enabled_gateways = CCP()->front->gateways->cl_get_enabled_payment_gateways();
		$cart_total       = CCP()->front->cart->get_total();
	...
	function cl_get_checkout_button_purchase_label() {
		if ( CCP()->front->cart->get_total() ) {
			$label             = cl_admin_get_option( 'checkout_label', '' );
			$complete_purchase = ! empty( $label ) ? $label : __( 'Purchase', 'clasify-classified-listing' );
		} else {
			$label             = cl_admin_get_option( 'free_checkout_label', '' );
			$complete_purchase = ! empty( $label ) ? $label : __( 'Free listing', 'clasify-classified-listing' );
		}
	...
	public function cl_checkout_form_shortcode() {
		ob_start();
		$gateways       = CCP()->front->gateways->cl_get_enabled_payment_gateways( true );
		$page_URL       = cl_get_current_page_url();
		$chosen_gateway = CCP()->front->gateways->cl_get_chosen_gateway();
		...
		<fieldset id="cl_cc_address" class="cc-address">
			...
			<p id="cl-card-address-wrap">
				<label for="card_address" class="cl-label">
					<?php _e( 'Billing Address', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'card_address' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description"><?php _e( 'The primary billing address for your credit card.', 'clasify-classified-listing' ); ?></span>
				...
				?>
				" placeholder="<?php _e( 'Address line 1', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr( $customer['address']['line1'] ); ?>" 
						<?php
						if ( $this->cl_field_is_required( 'card_address' ) ) {
							 echo ' required ';
						}
			...
			<p id="cl-card-address-2-wrap">
				<label for="card_address_2" class="cl-label">
					<?php _e( 'Billing Address Line 2 (optional)', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'card_address_2' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description"><?php _e( 'The suite, apt no, PO box, etc, associated with your billing address.', 'clasify-classified-listing' ); ?></span>
				...
				?>
				" placeholder="<?php _e( 'Address line 2', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr( $customer['address']['line2'] ); ?>" 
				<?php
				if ( $this->cl_field_is_required( 'card_address_2' ) ) {
					echo ' required ';
				}
			...
			<p id="cl-card-city-wrap">
				<label for="card_city" class="cl-label">
					<?php _e( 'Billing City', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'card_city' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description"><?php _e( 'The city for your billing address.', 'clasify-classified-listing' ); ?></span>
				...
				?>
				" placeholder="<?php _e( 'City', 'clasify-classified-listing' ); ?>" value="<?php echo esc_attr( $customer['address']['city'] ); ?>" 
				<?php
				if ( $this->cl_field_is_required( 'card_city' ) ) {
					echo ' required ';
				}
			...
			>
					<?php
					$selected_country = CCP()->front->country->cl_get_shop_country();

					if ( ! empty( $customer['address']['country'] ) && '*' !== $customer['address']['country'] ) {
						$selected_country = $customer['address']['country'];
					}

					$countries = CCP()->front->country->cl_get_country_list();
			...
			<p id="cl-card-state-wrap">
				<label for="card_state" class="cl-label">
					<?php _e( 'Billing State / Province', 'clasify-classified-listing' ); ?>
					<?php if ( $this->cl_field_is_required( 'card_state' ) ) { ?>
						<span class="cl-required-indicator">*</span>
					<?php } ?>
				</label>
				<span class="cl-description"><?php _e( 'The state or province for your billing address.', 'clasify-classified-listing' ); ?></span>
				<?php
				$selected_state = CCP()->front->country->cl_get_shop_state();
				$states         = CCP()->front->country->cl_get_shop_states( $selected_country );
				...
				<?php else : ?>
					...
					<input type="text" size="6" name="card_state" id="card_state" class="card_state cl-input" value="<?php echo esc_attr( $customer_state ); ?>" placeholder="<?php _e( 'State / Province', 'clasify-classified-listing' ); ?>" />
				<?php endif; ?>
			</p>
	...
	function cl_checkout_tax_fields() {
		if ( CCP()->front->tax->cl_cart_needs_tax_address_fields() && CCP()->front->cart->cl_get_cart_total() ) {
