...
class Fees {
	...
	public function add_fee( $args = array() ) {
		...
		// Sanitize the amount
		$args['amount'] = CCP()->common->formatting->cl_sanitize_amount( $args['amount'] );
		...
		// Update fees
		CCP()->front->session->set( 'cl_cart_fees', $fees );
	...
	public function has_fees( $type = 'fee' ) {
		...
		if ( 'all' == $type || 'fee' == $type ) {
			if ( ! CCP()->front->cart->cl_get_cart_contents() ) {
				$type = 'item';
			}
		}
	...
	public function get_fees( $type = 'fee', $listing_id = 0, $price_id = null ) {
		$fees = CCP()->front->session->get( 'cl_cart_fees' );

		if ( CCP()->front->cart->is_empty() ) {
	...
	public function type_total( $type = 'fee' ) {
		...
		if ( $this->has_fees( $type ) ) {
			foreach ( $fees as $fee ) {
				$total += CCP()->common->formatting->cl_sanitize_amount( $fee['amount'] );
			}
		}

		return CCP()->common->formatting->cl_sanitize_amount( $total );
	}
