<?php
namespace Clasify\Classified\Front\Purchase\Cart;

use Clasify\Classified\Front\Purchase\Cart\Cartactions;
...
class Cart {
	...
	public function get_contents_from_session() {
		$cart           = CCP()->front->session->get( 'cl_cart' );
		$this->contents = $cart;
	...
	public function get_discounts_from_session() {
		$discounts       = CCP()->front->session->get( 'cart_discounts' );
	...
	public function get_contents() {
		...
		foreach ( $cart as $key => $item ) {
			$isting = CCP()->front->listing_provider->get_details( $item['id'] );
	...
	public function get_contents_details() {
		...
		foreach ( $this->contents as $key => $item ) {
			...
			$item['quantity'] = CCP()->common->options->cl_item_quantities_enabled() ? absint( $item['quantity'] ) : 1;
			...
			if ( CCP()->front->tax->cl_prices_include_tax() ) {
				$subtotal -= round( $tax, CCP()->common->formatting->cl_currency_decimal_filter() );
			}
			...
			$details[ $key ] = array(
				...
				'id'          => $item['id'],
				'item_number' => $item,
				'item_price'  => round( $item_price, CCP()->common->formatting->cl_currency_decimal_filter() ),
				'quantity'    => $quantity,
				'discount'    => round( $discount, CCP()->common->formatting->cl_currency_decimal_filter() ),
				'subtotal'    => round( $subtotal, CCP()->common->formatting->cl_currency_decimal_filter() ),
				'tax'         => round( $tax, CCP()->common->formatting->cl_currency_decimal_filter() ),
				'fees'        => $fees,
				'price'       => round( $total, CCP()->common->formatting->cl_currency_decimal_filter() ),
			);
	...
	public function update_cart() {
		 CCP()->front->session->set( 'cl_cart', $this->contents );
	}
	...
	public function add( $listing_id, $options = array() ) {
		$isting = CCP()->front->listing_provider->get_details( $listing_id );
		if ( empty( $isting->ID ) ) {
			return; // Not a listing product
		}

		if ( ! CCP()->front->listing_provider->can_purchase() ) {
		...
		$quantities_enabled = CCP()->common->options->cl_item_quantities_enabled(); // && !cl_listing_quantities_disabled($listing_id);

		if ( CCP()->front->listing_provider->cl_has_variable_prices( $listing_id ) && ! isset( $options['price_id'] ) ) {
		...
		foreach ( $items as &$item ) {
			...
			if ( $this->is_item_in_cart( $to_add['id'], $to_add['options'] ) && CCP()->common->options->cl_item_quantities_enabled() ) {
		...
		CCP()->front->error->cl_clear_errors();
	...
	public function empty_cart() {
		...
		CCP()->front->session->set( 'cl_cart', null );

		// Remove all cart fees.
		CCP()->front->session->set( 'cl_cart_fees', null );

		// Remove any resuming payments.
		CCP()->front->session->set( 'cl_resume_payment', null );
	...
	public function remove_discount( $code = '' ) {
		...
		if ( $this->discounts ) {
			...
			CCP()->front->session->set( 'cart_discounts', $this->discounts );
		}
	...
	public function remove_all_discounts() {
		CCP()->front->session->set( 'cart_discounts', null );
	...
	public function get_item_discount_amount( $item = array(), $discount = false ) {
		...
		if ( ! empty( $discounts ) && 0.00 != $price ) {
			foreach ( $discounts as $discount ) {
				$code_id = CCP()->front->discountaction->cl_get_discount_id_by_code( $discount );
				...
				$reqs              = CCP()->front->discountaction->cl_get_discount_product_reqs( $code_id );
				$excluded_products = CCP()->front->discountaction->cl_get_discount_excluded_products( $code_id );
				...
				if ( ! empty( $reqs ) && cl_is_discount_not_global( $code_id ) ) {
					...
					if ( ! in_array( $item['id'], $excluded_products ) ) {
						if ( 'flat' === CCP()->front->discountaction->cl_get_discount_type( $code_id ) ) {
							/*
							 *
							...
							$code_amount       = CCP()->front->discountaction->cl_get_discount_amount( $code_id );
							$discounted_amount = $code_amount * $subtotal_percent;
							$discounted_price -= $discounted_amount;

							$cl_flat_discount_total += round( $discounted_amount, CCP()->common->formatting->cl_currency_decimal_filter() );
			...
			$amount = round( ( $price - apply_filters( 'cl_get_cart_item_discounted_amount', $discounted_price, $discounts, $item, $price ) ), CCP()->common->formatting->cl_currency_decimal_filter() );
	...
	public function display_cart_discount( $echo = false ) {
		...
		$discount_id = CCP()->front->discountaction->cl_get_discount_id_by_code( $discounts[0] );
		$amount      = CCP()->front->discountaction->cl_format_discount_rate( CCP()->front->discountaction->cl_get_discount_type( $discount_id ), CCP()->front->discountaction->CCP()->front->discountaction->cl_get_discount_amount( $discount_id ) );
	...
	public function get_item_quantity( $listing_id = 0, $options = array() ) {
		...
		$quantity = isset( $this->contents[ $key ]['quantity'] ) && CCP()->common->options->cl_item_quantities_enabled() ? $this->contents[ $key ]['quantity'] : 1;
	...
	public function item_price( $item_id = 0, $options = array() ) {
		...
		$listing  = CCP()->front->listing_provider->get_details( $item_id );

		if ( ! $listing->is_free && ! CCP()->front->tax->cl_listing_is_tax_exclusive( $item_id ) ) {
			if ( CCP()->front->tax->cl_prices_show_tax_on_checkout() && ! CCP()->front->tax->cl_prices_include_tax() ) {
			...
			if ( CCP()->front->tax->cl_display_tax_rate() ) {
				$label = '&nbsp;&ndash;&nbsp;';

				if ( CCP()->front->tax->cl_prices_show_tax_on_checkout() ) {
					$label .= sprintf( __( 'includes %s tax', 'clasify-classified-listing' ), CCP()->front->tax->cl_get_formatted_tax_rate() );
				} else {
					$label .= sprintf( __( 'excludes %s tax', 'clasify-classified-listing' ), CCP()->front->tax->cl_get_formatted_tax_rate() );
				}
		...
		$price = CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $price ) );
	...
	public function get_item_price( $listing_id = 0, $options = array(), $remove_tax_from_inclusive = false ) {
		$price           = 0;
		$variable_prices = CCP()->front->listing_provider->cl_has_variable_prices( $listing_id );
		...
		if ( ! $variable_prices || false === $price ) {
			...
			$price = CCP()->front->listing_provider->get_price( $listing_id );
		}

		if ( $remove_tax_from_inclusive && CCP()->front->tax->cl_prices_include_tax() ) {
	...
	public function get_item_tax( $listing_id = 0, $options = array(), $subtotal = '' ) {
		...
		if ( ! CCP()->front->tax->cl_listing_is_tax_exclusive( $listing_id ) ) {
			...
			$tax = CCP()->front->tax->cl_calculate_tax( $subtotal, $country, $state );
		}
	...
	public function get_fees( $type = 'all', $listing_id = 0, $price_id = null ) {
		return CCP()->front->fees->get_fees( $type, $listing_id, $price_id );
	}
	...
	public function get_all_fees() {
		$this->fees = CCP()->front->fees->get_fees( 'all' );
		return $this->fees;
	}
	...
	public function subtotal() {
		return esc_html( CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $this->get_subtotal() ) ) );
	}
	...
	public function total( $echo ) {
		$total = apply_filters( 'cl_cart_total', CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $this->get_total() ) ) );
	...
	public function get_item_name( $item = array() ) {
		...
		if ( CCP()->front->listing_provider->cl_has_variable_prices( $item['id'] ) && false !== $this->get_item_price_id( $item ) ) {
	...
	public function get_tax() {
		...
		$cart_tax = apply_filters( 'cl_get_cart_tax', CCP()->common->formatting->cl_sanitize_amount( $cart_tax ) );
	...
	public function get_tax_on_fees() {
		...
		if ( $fees ) {
			foreach ( $fees as $fee_id => $fee ) {
				...
				$tax += CCP()->front->tax->cl_calculate_tax( $fee['amount'] );
	...
	public function is_saved() {
		...
		if ( is_user_logged_in() ) {
			...
			if ( $saved_cart === CCP()->front->session->get( 'cl_cart' ) ) {
				return false;
			}
			...
			if ( json_decode( stripslashes( $_COOKIE['cl_saved_cart'] ), true ) === CCP()->front->session->get( 'cl_cart' ) ) {
				return false;
			}
	...
	public function save() {
		...
		$messages = CCP()->front->session->get( 'cl_cart_messages' );
		...
		$messages['cl_cart_save_successful'] = sprintf(
			...
			__( 'Success', 'clasify-classified-listing' ),
			__( 'Cart saved successfully. You can restore your cart using this URL:', 'clasify-classified-listing' ) . ' ' . '<a href="' . esc_url( cl_get_checkout_uri() ) . '?cl_action=restore_cart&cl_cart_token=' . esc_attr( $token ) . '">' . esc_url( cl_get_checkout_uri() ) . '?cl_action=restore_cart&cl_cart_token=' . esc_attr( $token ) . '</a>'
		);

		CCP()->front->session->set( 'cl_cart_messages', $messages );
	...
	public function restore() {
		...
		if ( is_user_logged_in() && $saved_cart ) {
			...
			if ( isset( $_GET['cl_cart_token'] ) && ! hash_equals( $_GET['cl_cart_token'], $token ) ) {
				$messages['cl_cart_restoration_failed'] = sprintf( '<strong>%1$s</strong>: %2$s', __( 'Error', 'clasify-classified-listing' ), __( 'Cart restoration failed. Invalid token.', 'clasify-classified-listing' ) );
				CCP()->front->session->set( 'cl_cart_messages', $messages );
			}
			...
			if ( isset( $_GET['cl_cart_token'] ) && $_GET['cl_cart_token'] != $token ) {
				return new WP_Error( 'invalid_cart_token', __( 'The cart cannot be restored. Invalid token.', 'clasify-classified-listing' ) );
			}
		...
		$messages['cl_cart_restoration_successful'] = sprintf( '<strong>%1$s</strong>: %2$s', __( 'Success', 'clasify-classified-listing' ), __( 'Cart restored successfully.', 'clasify-classified-listing' ) );
		CCP()->front->session->set( 'cl_cart', $saved_cart );
		CCP()->front->session->set( 'cl_cart_messages', $messages );
	...
	function cl_cart_has_fees() {
		return CCP()->front->fees->has_fees();
	}
	...
	function cl_get_cart_tax_rate( $country = '', $state = '', $postal_code = '' ) {
		$rate = CCP()->front->tax->cl_get_tax_rate( $country, $state );
	...
	function cl_set_purchase_session( $purchase_data = array() ) {
		CCP()->front->session->set( 'cl_purchase', $purchase_data );
	}
	function cl_get_cart_item_name( $item = array() ) {
		return CCP()->front->cart->get_item_name( $item );
	}
