...
class Cartactions {
	...
	public function cl_process_add_to_cart( $data ) {
		...
		if ( CCP()->common->options->cl_straight_to_checkout() && ! cl_is_checkout() ) {
	...
	public function cl_process_remove_from_cart( $data ) {
		...
		if ( ! isset( $_GET['cl_remove_from_cart_nonce'] ) ) {
			CCP()->front->error->cl_set_error( 'remove_from_cart', sprintf( __( 'Security key not found', 'clasify-classified-listing' ) ) );
			CCP()->front->error->cl_print_errors();
		}
		...
		if ( false !== $nonce_verified ) {
			CCP()->front->cart->remove( $cart_key );
		} else {
			CCP()->front->error->cl_set_error( 'remove_from_cart_error', sprintf( __( 'Security key not verified', 'clasify-classified-listing' ) ) );
			CCP()->front->error->cl_print_errors();
		}
	...
	public function append_cart_button( $args ) {
		...
		if ( ! $purchase_page || $purchase_page == 0 ) {
			CCP()->front->error->cl_set_error( 'set_checkout', sprintf( __( 'No checkout page has been configured. Visit <a href="%s">Settings</a> to set one.', 'clasify-classified-listing' ), admin_url( 'edit.php?post_type=cl_cpt&page=listing_settings_func' ) ) );
			CCP()->front->error->cl_print_errors();
			return false;
		}

		$post_id = is_object( $post ) ? $post->ID : 0;
		$listing = CCP()->front->listing_provider->listing;
		if ( isset( $args['post_id'] ) ) {
			$post_id = $args['post_id'];
			$listing = CCP()->front->listing_provider->get_details( $post_id );
		}
		...
		$defaults = apply_filters(
			...
			array(
				...
				'text'       => ( isset( $listing->button_behavior ) && $listing->button_behavior == 'direct' ) ? cl_admin_get_option( 'buy_now_text', __( 'Buy Now', 'clasify-classified-listing' ) ) : cl_admin_get_option( 'add_to_cart_text', __( 'Purchase', 'clasify-classified-listing' ) ),
				'checkout'   => cl_admin_get_option( 'checkout_button_text', _x( 'Checkout', 'text shown on the Add to Cart Button when the product is already in the cart', 'clasify-classified-listing' ) ),
		...
		if ( ! CCP()->front->gateways->cl_shop_supports_buy_now() ) {
			$args['direct'] = false;
		}
		if ( empty( $listing->ID ) ) {
			return false;
		...
		if ( false !== $price ) {
			...
			if ( 0 == $price ) {
				$args['text'] = '<span>' . __( 'Free', 'clasify-classified-listing' ) . '</span>' . $button_text;
			} else {
				$args['text'] = '<span>' . CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $price ) ) . '</span>' . $button_text;
				$args['text'] = apply_filters( 'cl_purchase_listing_cart_button_text', $args['text'] );
			}
		}

		if ( CCP()->front->cart->is_item_in_cart( $listing->ID, $options ) && ( ! $variable_pricing || ! $listing->is_single_price_mode ) ) {
	...
	public function cl_get_cart_item_template_func( $cart_key, $item, $ajax = false ) {
		...
		$listing    = CCP()->front->listing_provider->get_details( $id );
		$remove_url = CCP()->front->cart->remove_item_url( $cart_key );
		$title      = $listing->title;
		$options    = ! empty( $item['options'] ) ? $item['options'] : array();
		$quantity   = CCP()->front->cart->get_item_quantity( $id, $options );
		$price      = CCP()->front->cart->get_item_price( $id, $options );

		if ( ! empty( $options ) ) {
			$title .= ( CCP()->front->listing_provider->cl_has_variable_prices( $item['id'] ) ) ? ' <span class="cl-cart-item-separator">-</span> ' . CCP()->front->listing_provider->cl_get_price_name( $id, $item['options'] ) : CCP()->front->listing_provider->cl_get_price_name( $id, $item['options'] );
		}
		ob_start();
		cl_get_template( 'cart/cart-item.php' );
		$item     = ob_get_clean();
		$item     = str_replace( '{item_title}', $title, $item );
		$item     = str_replace( '{item_amount}', CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $price ) ), $item );
		...
		$subtotal = '';
		if ( $ajax ) {
			$subtotal = CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( CCP()->front->cart->get_subtotal() ) );
		}
	...
	function cl_empty_cart_message() {
		return apply_filters( 'cl_empty_cart_message', '<span class="cl_empty_cart">' . __( 'Your cart is empty.', 'clasify-classified-listing' ) . '</span>' );
	}

	function cl_empty_checkout_cart() {
		 echo CCP()->front->cart->cl_empty_cart_message();
	}
}
