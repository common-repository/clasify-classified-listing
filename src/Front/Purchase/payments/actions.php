...
function cl_update_payment_details( $data ) {
	...
	$curr_total = CCP()->common->formatting->cl_sanitize_amount( $payment->total );
	$new_total  = CCP()->common->formatting->cl_sanitize_amount( cl_sanitization( $_POST['cl-payment-total'] ) );
	$tax        = isset( $_POST['cl-payment-tax'] ) ? CCP()->common->formatting->cl_sanitize_amount( cl_sanitization( $_POST['cl-payment-tax'] ) ) : 0;
	...
	if ( $updated_listings ) {
		...
		foreach ( $updated_listings as $cart_position => $listing ) {
			...
			if ( $has_log ) {
				...
				$item_price = CCP()->common->formatting->cl_format_amount( $item_price );
				$item_tax   = CCP()->common->formatting->cl_format_amount( $item_tax );
				...
				if ( CCP()->front->listing_provider->cl_has_variable_prices( $listing_id ) && isset( $listing['price_id'] ) ) {
	...
	if ( isset( $data['cl-new-customer'] ) && $data['cl-new-customer'] == '1' ) {
		...
		if ( empty( $email ) || empty( $names ) ) {
			wp_die( __( 'New Customers require a name and email address', 'clasify-classified-listing' ) );
		}
		...
		if ( empty( $customer->id ) ) {
			...
			if ( ! $customer->create( $customer_data ) ) {
				...
				CCP()->front->error->cl_set_error( 'cl-payment-new-customer-fail', __( 'Error creating new customer', 'clasify-classified-listing' ) );
			}
		} else {
			wp_die( sprintf( __( 'A customer with the email address %s already exists. Please go back and use the "Assign to another customer" link to assign this payment to them.', 'clasify-classified-listing' ), $email ) );
		}
	...
	if ( 0 === $updated ) {
		wp_die( __( 'Error Updating Payment', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 400 ) );
	}
...
function cl_process_after_payment_actions( $payment_id = 0, $force = false ) {
	...
	$payment->add_note( __( 'After payment actions processed.', 'clasify-classified-listing' ) );
...
function cl_record_status_change( $payment_id, $new_status, $old_status ) {
	...
	$status_change = sprintf( __( 'Status changed from %1$s to %2$s', 'clasify-classified-listing' ), $old_status, $new_status );
...
function cl_recover_payment() {
	...
	if (
		...
		CCP()->front->error->cl_set_error( 'cl-payment-recovery-user-mismatch', __( 'Error resuming payment.', 'clasify-classified-listing' ) );
		wp_redirect( $redirect );
	}

	$payment->add_note( __( 'Payment recovery triggered URL', 'clasify-classified-listing' ) );

	// Empty out the cart.
	CCP()->front->cart->empty_cart();
	...
	if ( 'none' !== $payment->discounts && ! empty( $payment->discounts ) ) {
		...
		foreach ( $discounts as $discount ) {
			CCP()->front->discountaction->cl_set_cart_discount( $discount );
		}
	}

	CCP()->front->session->set( 'cl_resume_payment', $payment->ID );
...
function cl_recovery_user_mismatch() {
	...
	$resuming_payment = CCP()->front->session->get( 'cl_resume_payment' );
	if ( $resuming_payment ) {
...
function cl_recovery_force_login_fields() {
	...
	if ( $resuming_payment ) {
		$payment        = new Clpayment( $resuming_payment );
		$requires_login = CCP()->common->options->cl_no_guest_checkout();
		if ( ( $requires_login && ! is_user_logged_in() ) && ( $payment->user_id > 0 && ( ! is_user_logged_in() ) ) ) {
			?>
			<div class="cl-alert cl-alert-info">
				<p><?php _e( 'To complete this payment, please login to your account.', 'clasify-classified-listing' ); ?></p>
				<p>
					<a href="<?php echo esc_url( cl_get_lostpassword_url() ); ?>" title="<?php esc_attr_e( 'Lost Password', 'clasify-classified-listing' ); ?>">
						<?php _e( 'Lost Password?', 'clasify-classified-listing' ); ?>
					</a>
				</p>
			</div>
			<?php
...
function cl_recovery_verify_logged_in( $verified_data, $post_data ) {
	...
	if ( $resuming_payment ) {
		...
		if ( ( is_user_logged_in() && ! $same_user ) || ( ! is_user_logged_in() && (int) $payment->user_id > 0 && ! $same_email ) ) {
			CCP()->front->error->cl_set_error( 'recovery_requires_login', __( 'To complete this payment, please login to your account.', 'clasify-classified-listing' ) );
		}
	}
}
