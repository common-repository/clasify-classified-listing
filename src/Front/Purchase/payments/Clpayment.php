<?php
namespace Clasify\Classified\Front\Purchase\Payments;
...
class Clpayment {
								...
								case 'add':
									...
									if ( 'publish' === $this->status || 'complete' === $this->status || 'revoked' === $this->status ) {
										...
										while ( $y < $item['quantity'] ) {
											CCP()->front->listingsaction->cl_record_sale_in_log( $item['id'], $this->ID, $price_id, $log_date );
											$y++;
										}
	...
	public function add_listing( $listing_id = 0, $args = array(), $options = array() ) {
		...
		$item_price = CCP()->common->formatting->cl_sanitize_amount( $item_price );
		$quantity   = cl_item_quantities_enabled() ? absint( $args['quantity'] ) : 1;
		$amount     = round( $item_price * $quantity, CCP()->common->formatting->cl_currency_decimal_filter() );
		...
		if ( ! empty( $args['fees'] ) && is_array( $args['fees'] ) ) {
			...
			$fees = round( $fees, CCP()->common->formatting->cl_currency_decimal_filter() );
		}
	...
	public function remove_listing( $listing_id, $args = array() ) {
		...
		if ( $orig_quantity > $args['quantity'] ) {
			...
			$total_reduced = round( $item_price * $args['quantity'], CCP()->common->formatting->cl_currency_decimal_filter() );
			$tax_reduced   = round( ( $tax / $orig_quantity ) * $args['quantity'], CCP()->common->formatting->cl_currency_decimal_filter() );
	...
	public function modify_cart_item( $cart_index = false, $args = array() ) {
		...
		$merged_item['item_price'] = CCP()->common->formatting->cl_sanitize_amount( $merged_item['item_price'] );
		$new_subtotal              = floatval( $merged_item['item_price'] ) * $merged_item['quantity'];
		$merged_item['tax']        = CCP()->common->formatting->cl_sanitize_amount( $merged_item['tax'] );
		$merged_item['price']      = CCP()->front->tax->cl_prices_include_tax() ? $new_subtotal : $new_subtotal + $merged_item['tax'];
	...
	private function setup_subtotal() {
		...
		if ( is_array( $cart_details ) ) {
			...
			$subtotal  = $this->total;
			$tax       = CCP()->front->tax->cl_use_taxes() ? $this->tax : 0;
			$subtotal -= $tax;
		}
	...
	private function setup_currency() {
		 $currency = isset( $this->payment_meta['currency'] ) ? $this->payment_meta['currency'] : apply_filters( 'cl_payment_currency_default', CCP()->common->options->cl_get_currency(), $this );
		return $currency;
	}
	...
	private function setup_email() {
		...
		if ( empty( $email ) ) {
			$email = CCP()->common->dbcustomer->get_column( 'email', $this->customer_id );
		}
