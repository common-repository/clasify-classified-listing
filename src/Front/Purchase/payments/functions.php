...
function cl_insert_payment( $payment_data = array() ) {
	...
	if ( $resume_payment ) {
		...
		$payment->add_note( __( 'Payment recovery processed', 'clasify-classified-listing' ) );
	...
	$payment->currency       = ! empty( $payment_data['currency'] ) ? $payment_data['currency'] : CCP()->common->options->cl_get_currency();
	...
	$payment->tax_rate       = CCP()->front->cart->cl_get_cart_tax_rate( $country, $state, $zip );
		...
		foreach ( $cart_details as $item ) {
			...
			for ( $i = 0; $i < $item['quantity']; $i++ ) {
				...
				if ( ! $amount ) {
					...
					$amount = cl_get_listing_final_price( $item['id'], $user_info, $amount );
				}
			}
...
function cl_get_payment_statuses() {
	$payment_statuses = array(
		...
		'publish'    => __( 'Complete', 'clasify-classified-listing' ),
		'refunded'   => __( 'Refunded', 'clasify-classified-listing' ),
		'failed'     => __( 'Failed', 'clasify-classified-listing' ),
		'abandoned'  => __( 'Abandoned', 'clasify-classified-listing' ),
		'revoked'    => __( 'Revoked', 'clasify-classified-listing' ),
		'processing' => __( 'Processing', 'clasify-classified-listing' ),
	);
...
function cl_payment_amount( $payment_id = 0 ) {
	$amount = cl_get_payment_amount( $payment_id );
	return CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $amount ), cl_get_payment_currency_code( $payment_id ) );
}
...
function cl_payment_subtotal( $payment_id = 0 ) {
	...
	return CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $subtotal ), cl_get_payment_currency_code( $payment_id ) );
}
...
function cl_payment_tax( $payment_id = 0, $payment_meta = false ) {
	...
	return CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $tax ), cl_get_payment_currency_code( $payment_id ) );
}
...
function cl_get_payment_note_html( $note, $payment_id = 0 ) {
	...
	if ( ! empty( $note->user_id ) ) {
		...
		$user = __( 'CL BOT', 'clasify-classified-listing' );
	}
	...
	$note_html .= '&nbsp;&ndash;&nbsp;<a href="' . esc_url( $delete_note_url ) . '" class="cl-delete-payment-note" data-note-id="' . absint( $note->comment_ID ) . '" data-payment-id="' . absint( $payment_id ) . '">' . __( 'Delete', 'clasify-classified-listing' ) . '</a>';
	$note_html .= '</p>';
	$note_html .= '</div>';
