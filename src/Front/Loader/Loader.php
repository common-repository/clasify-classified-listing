...
use Clasify\Classified\Front\Loader\Styles;
...
class Loader extends Styles {
	...
	public function listing_loop( $args ) {
		CCP()->front->listing_provider->set_listing_object();
	...
	public function single_section_loop() {
		...
		$section_data = CCP()->front->listing_provider->single_listing_section();
	...
	public function section_content_func( $section ) {
		...
		foreach ( $section['blocks'] as $block ) {
			if ( $block['type'] == 'other' ) {
				$value = CCP()->front->listing_provider->get_meta_data( $block['key'], get_the_ID() );
	...
	public function comment_ratings( $comment_id, $comment_approved, $commentdata ) {
		...
		$average         = CCP()->front->listing_provider->get_average_rate( $comment_post_id );
