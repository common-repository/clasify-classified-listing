...
class Shortcode {
	...
	function cl_cart_shortcode( $atts, $content = null ) {
		return CCP()->front->cart->cl_shopping_cart();
	}

	function cl_checkout_form_shortcode( $atts, $content = null ) {
		return CCP()->front->checkout->cl_checkout_form_shortcode();
	}
	...
	function cl_receipt_shortcode( $atts, $content = null ) {
		...
		$cl_receipt_args = shortcode_atts(
			array(
				'error'          => __( 'Sorry, trouble retrieving payment receipt.', 'clasify-classified-listing' ),
				'price'          => true,
				'discount'       => true,
				'products'       => true,
				'date'           => true,
		...
		if ( ! $user_can_view && ! empty( $payment_key ) && ! is_user_logged_in() && ! cl_is_guest_payment( $payment_id ) ) {
			...
			echo '<p class="cl-alert cl-alert-warn">' . __( 'You must be logged in to view this payment receipt.', 'clasify-classified-listing' ) . '</p>';
		...
		if ( empty( $session ) && ! is_user_logged_in() && ! $user_can_view ) {
			return '<p class="cl-alert cl-alert-error">' . apply_filters( 'cl_receipt_guest_error_message', __( 'Receipt could not be retrieved, your purchase session has expired.', 'clasify-classified-listing' ) ) . '</p>';
		}
	...
	function cl_listing_shortcode( $atts, $content = null ) {
		...
		if ( ! $atts['text'] ) {
			if ( $atts['direct'] == '1' || $atts['direct'] == 'true' ) {
				$atts['text'] = cl_admin_get_option( 'buy_now_text', __( 'Buy Now', 'clasify-classified-listing' ) );
			} else {
				$atts['text'] = cl_admin_get_option( 'add_to_cart_text', __( 'Purchase', 'clasify-classified-listing' ) );
			}
		}
	...
	public function failure_page_func( $args = array() ) {
		return '<p>' . esc_html__( 'Your transaction failed, please try again or contact site support.', 'clasify-classified-listing' ) . '</p>';
	}
	...
	public function cancel_payment_func( $args = array() ) {
		return '<p>' . esc_html__( 'Your transaction has been successfully cancelled.', 'clasify-classified-listing' ) . '</p>';
	}
	...
	public function purchase_history_func( $args = array() ) {
		...
		if ( ! CCP()->common->user->cl_user_pending_verification() ) {
