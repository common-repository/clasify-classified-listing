<?php
namespace Clasify\Classified\Front\Error;
...
class Error {
	...
	function cl_print_errors() {
		...
		if ( $errors ) {
			...
			foreach ( $errors as $error_id => $error ) {
				echo '<p class="cl_error" id="cl_error_' . esc_attr( $error_id ) . '"><strong>' . __( 'Error', 'clasify-classified-listing' ) . '</strong>: ' . esc_html( $error ) . '</p>';
			}
			echo '</div>';
	...
	function cl_get_errors() {
		return CCP()->front->session->get( 'cl_errors' );
	}
	...
	function cl_set_error( $error_id, $error_message ) {
		...
		CCP()->front->session->set( 'cl_errors', $errors );
	}
	...
	function cl_clear_errors() {
		CCP()->front->session->set( 'cl_errors', null );
	}
