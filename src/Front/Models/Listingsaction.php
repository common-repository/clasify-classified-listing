<?php
namespace Clasify\Classified\Front\Models;
...
class Listingsaction {
	...
	function cl_price( $listing_id = 0, $echo = true, $price_id = false ) {
		...
		if ( $this->cl_has_variable_prices( $listing_id ) ) {
			...
			$price = CCP()->common->formatting->cl_sanitize_amount( $price );
		} else {

			$price = $this->cl_get_listing_price( $listing_id );
		}

		$price           = apply_filters( 'cl_listing_price', CCP()->common->formatting->cl_sanitize_amount( $price ), $listing_id, $price_id );
	...
	function cl_get_listing_final_price( $listing_id, $user_purchase_info, $amount_override = null ) {
		...
		if ( isset( $user_purchase_info['discount'] ) && $user_purchase_info['discount'] != 'none' ) {
			...
			if ( CCP()->front->discountaction->cl_get_discount_type( CCP()->front->discountaction->cl_get_discount_id_by_code( $user_purchase_info['discount'] ) ) != 'flat' ) {
				$price = CCP()->front->discountaction->cl_get_discounted_amount( $user_purchase_info['discount'], $original_price );
			} else {
	...
	function cl_get_price_option_amount( $listing_id = 0, $price_id = 0 ) {
		...
		return apply_filters( 'cl_get_price_option_amount', CCP()->common->formatting->cl_sanitize_amount( $amount ), $listing_id, $price_id );
	}
	...
	function cl_get_lowest_price_option( $listing_id = 0 ) {
		...
		return CCP()->common->formatting->cl_sanitize_amount( $low );
	}
	...
	function cl_get_highest_price_option( $listing_id = 0 ) {
		...
		return CCP()->common->formatting->cl_sanitize_amount( $high );
	}
	...
	function cl_price_range( $listing_id = 0 ) {
		...
		$range  = '<span class="cl_price cl_price_range_low" id="cl_price_low_' . esc_attr( $listing_id ) . '">' . CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $low ) ) . '</span>';
		$range .= '<span class="cl_price_range_sep">&nbsp;&ndash;&nbsp;</span>';
		$range .= '<span class="cl_price cl_price_range_high" id="cl_price_high_' . esc_attr( $listing_id ) . '">' . CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $high ) ) . '</span>';
	...
	function cl_get_listing_types() {
		...
		$types = array(
			'0'      => __( 'Default', 'clasify-classified-listing' ),
			'bundle' => __( 'Bundle', 'clasify-classified-listing' ),
		);
	...
	function cl_validate_url_token( $url = '' ) {
		...
		if ( isset( $parts['query'] ) ) {
			...
			if ( isset( $query_args['ttl'] ) && current_time( 'timestamp' ) > $query_args['ttl'] ) {
				...
				wp_die( apply_filters( 'cl_listing_link_expired_text', __( 'Sorry but your listing link has expired.', 'clasify-classified-listing' ) ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
			}
