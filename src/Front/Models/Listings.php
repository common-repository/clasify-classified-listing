...
use Clasify\Classified\Front\Models\Datacore;
...
class Listings extends Datacore {
	...
	protected function output_thumb( $args ) {
		...
		if ( isset( $this->listing->ID ) && $this->listing->ID ) {
			...
			if ( isset( $img_url ) && $img_url ) {
				if ( $img_class != '' ) {
					return sprintf( '<img class="%1s" id="%2s" src="%3s" alt="%4s">', esc_attr( $img_class ), esc_attr( $this->prefix . 'thumb_id_' . $this->ID ), esc_url( $img_url ), esc_attr( $img_alt ) );
				} else {
					return sprintf( '<img id="%1s" src="%2s" alt="%3s">', esc_attr( $this->prefix . 'thumb_id_' . $this->ID ), esc_url( $img_url ), esc_attr( $img_alt ) );
				}
			}
			return false;
		}
	...
	public function get_prices( $listing_id = 0 ) {
		...
		if ( true === $this->has_variable_prices() ) {
			...
			if ( empty( $this->prices ) ) {
				$this->prices = get_post_meta( $listing_id, 'clasify_classified_plugin_pricing', true );
			}
		}
	...
	public function get_price( $listing_id = 0 ) {
		...
		if ( ! empty( $listing_id ) ) {
			...
			$this->price = get_post_meta( $listing_id, 'clasify_classified_plugin_pricing', true );

			if ( $this->price ) {

				$this->price = CCP()->common->formatting->cl_sanitize_amount( $this->price );
			} else {
