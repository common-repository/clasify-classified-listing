<?php
use Clasify\Classified\Front\cl_Logging\cl_Logging;
...
if ( ! function_exists( 'cl_template_path' ) ) {
	function cl_template_path() {
		return apply_filters( 'cl_template_path', 'clasify-classified-listing' );
	}
}
...
if ( ! function_exists( 'cl_get_template_with_dir' ) ) {
	...
	function cl_get_template_with_dir( $slug, $dir = '' ) {
		...
		// Look in yourtheme/slug-name and yourtheme/clasify-classified-listing/slug-name
		if ( $dir ) {
			$template = locate_template( array( "{$dir}/{$slug}", cl_template_path() . "/{$dir}/{$slug}" ) );
		}
		// Get default slug-name
		if ( ! $template && $dir && file_exists( CLASIFY_CLASSIFIED_PLUGIN_FRONT_TEMPLATE_DIR . "/listings/{$dir}/{$slug}" ) ) {
			$template = CLASIFY_CLASSIFIED_PLUGIN_FRONT_TEMPLATE_DIR . "/listings/{$dir}/{$slug}";
		}
		// If template file doesn't exist, look in yourtheme/slug and yourtheme/clasify-classified-listing/slug
		if ( ! $template ) {
...
if ( ! function_exists( 'cl_get_template_part' ) ) {
	...
	function cl_get_template_part( $slug, $name = '' ) {
		...
		// Get default slug-name
		if ( ! $template && $name && file_exists( CLASIFY_CLASSIFIED_PLUGIN_FRONT_TEMPLATE_DIR . "/listings/{$slug}-{$name}" ) ) {
			$template = CLASIFY_CLASSIFIED_PLUGIN_FRONT_TEMPLATE_DIR . "/listings/{$slug}-{$name}";
		}
		// If template file doesn't exist, look in yourtheme/slug and yourtheme/clasify-classified-listing/slug
		if ( ! $template ) {
...
if ( ! function_exists( 'cl_locate_template' ) ) {
	...
	function cl_locate_template( $template_name, $template_path = '', $default_path = '' ) {
		...
		if ( ! $default_path ) {
			$default_path = CLASIFY_CLASSIFIED_PLUGIN_FRONT_TEMPLATE_DIR . '/listings/';
		}
		$template = null;
...
function cl_get_theme_template_paths() {
	...
	$file_paths = array(
		...
		100 => CLASIFY_CLASSIFIED_PLUGIN_ASSETS_CSS_DIR,
	);
...
// add_action('update_option_cl_admin_settings', 'init_clasify_classified_plugin_pages');

// -- Trigger only when Plugin activated
register_activation_hook( CLASIFY_CLASSIFIED_PLUGIN_FILE, 'init_clasify_classified_plugin_pages' );

/**
 * init_clasify_classified_plugin_pages
 *
 * @return void
 * @since 1.0.0
 */
function init_clasify_classified_plugin_pages() {
...
add_filter( 'display_post_states', 'init_clasify_classified_plugin_pages_state', 10, 2 );
/**
 * init_clasify_classified_plugin_pages_state
 *
 ...
 * @since 1.0.0
 */
function init_clasify_classified_plugin_pages_state( $post_states, $post ) {
	$cl_pages    = get_option( 'cl_admin_settings' );
	$post_states = array();
	if ( $cl_pages ) {
...
function cl_get_default_labels() {
	$defaults = array(
		'singular' => __( 'listing', 'clasify-classified-listing' ),
		'plural'   => __( 'listings', 'clasify-classified-listing' ),
	);
...
function cl_is_cc_verify_enabled() {
	...
	$gateways = CCP()->front->gateways->cl_get_enabled_payment_gateways();
...
function cl_get_htaccess_rules( $method = false ) {
	...
	if ( empty( $method ) ) {
		$method = CCP()->front->listingsaction->cl_get_file_listing_method();
	}
...
function cl_listing_insert_enquiry_message( $args = array() ) {
	global $wpdb;
	if ( empty( $args['name'] ) ) {
		return new \WP_Error( 'no-name', __( 'You must provide a name.', 'clasify-classified-listing' ) );
	}
	...
	if ( ! $inserted ) {
		return new \WP_Error( 'failed-to-insert', __( 'Failed to insert data', 'clasify-classified-listing' ) );
	}
