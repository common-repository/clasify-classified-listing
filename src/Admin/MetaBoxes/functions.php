...
function meta_clone_variable_declaration( $param_clone, $param_id, $param_val ) {
	...
	if ( isset( $param_clone ) && $param_clone === true ) {
		...
		$after_clone  = '</div>';
		$cl_name      = $param_id . '[]';
		$add_btn      = '<button id="' . esc_attr( $param_id ) . '" class="mb_btn primary clone_btn"><span class="dashicons dashicons-plus-alt"></span>' . esc_html__( 'Add New', 'clasify-classified-listing' ) . '</button>';
