...
function cl_add_discount( $data ) {
	...
	if ( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( __( 'You do not have permission to create discount codes', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}
	...
	if ( ! CCP()->admin->discount_action->cl_get_discount_by_code( $posted['code'] ) ) {

		// Set the discount code's default status to active
		$posted['status'] = 'active';

		if ( CCP()->admin->discount_action->cl_store_discount( $posted ) ) {
...
function cl_edit_discount( $data ) {
	...
	if ( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( __( 'You do not have permission to edit discount codes', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}
	...
	if ( CCP()->admin->discount_action->cl_store_discount( $discount, $data['discount-id'] ) ) {
...
function cl_delete_discount( $data ) {
	...
	if ( ! isset( $data['_wpnonce'] ) || ! wp_verify_nonce( $data['_wpnonce'], 'cl_discount_nonce' ) ) {
		wp_die( __( 'Trying to cheat or something?', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}

	if ( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( __( 'You do not have permission to delete discount codes', 'clasify-classified-listing' ), __( 'Error', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}

	$discount_id = $data['discount'];
	CCP()->front->discountaction->cl_remove_discount( $discount_id );
}
...
function cl_deactivate_discount( $data ) {
	...
	if ( ! current_user_can( 'manage_shop_discounts' ) ) {
		wp_die( __( 'You do not have permission to create discount codes', 'clasify-classified-listing' ), array( 'response' => 403 ) );
	}
