<?php
namespace Clasify\Classified\Admin\Settings\Payment;
...
class Paymentlist extends \WP_List_Table {
	...
	public function advanced_filters() {
		...
		if ( ! empty( $all_gateways ) ) {
			$gateways['all'] = __( 'All Gateways', 'clasify-classified-listing' );
		...
		<div id="cl-payment-filters">
			<span id="cl-payment-date-filters">
				<span>
					<label for="start-date"><?php _e( 'Start Date:', 'clasify-classified-listing' ); ?></label>
				...
				<span>
					<label for="end-date"><?php _e( 'End Date:', 'clasify-classified-listing' ); ?></label>
			...
			<span id="cl-payment-after-core-filters">
				...
				<input type="submit" class="button-secondary" value="<?php _e( 'Apply', 'clasify-classified-listing' ); ?>" />
			</span>
			<?php if ( ! empty( $status ) ) : ?>
				<input type="hidden" name="status" value="<?php echo esc_attr( $status ); ?>" />
			<?php endif; ?>
			<?php if ( ! empty( $start_date ) || ! empty( $end_date ) || 'all' !== $selected_gateway ) : ?>
				<a href="<?php echo admin_url( 'edit.php?post_type=cl_cpt&page=cl-payment-history' ); ?>" class="button-secondary"><?php _e( 'Clear Filter', 'clasify-classified-listing' ); ?></a>
			<?php endif; ?>
			<?php do_action( 'cl_payment_advanced_filters_row' ); ?>
			<?php $this->search_box( __( 'Search', 'clasify-classified-listing' ), 'cl-payments' ); ?>
		</div>
	...
	public function get_views() {
		...
		$views = array(
			'all'        => sprintf( '<a href="%s"%s>%s</a>', remove_query_arg( array( 'status', 'paged' ) ), $current === 'all' || $current == '' ? ' class="current"' : '', __( 'All', 'clasify-classified-listing' ) . $total_count ),
			'publish'    => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
					array(
				...
				__( 'Completed', 'clasify-classified-listing' ) . $complete_count
			),
			'pending'    => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
				...
				__( 'Pending', 'clasify-classified-listing' ) . $pending_count
			),
			'processing' => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
				...
				__( 'Processing', 'clasify-classified-listing' ) . $processing_count
			),
			'refunded'   => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
				...
				__( 'Refunded', 'clasify-classified-listing' ) . $refunded_count
			),
			'revoked'    => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
				...
				__( 'Revoked', 'clasify-classified-listing' ) . $revoked_count
			),
			'failed'     => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
				...
				__( 'Failed', 'clasify-classified-listing' ) . $failed_count
			),
			'abandoned'  => sprintf(
				'<a href="%s"%s>%s</a>',
				add_query_arg(
				...
				__( 'Abandoned', 'clasify-classified-listing' ) . $abandoned_count
			),
		);
	...
	public function get_columns() {
		 $columns = array(
			 ...
			 'ID'       => __( 'ID', 'clasify-classified-listing' ),
			 'email'    => __( 'Email', 'clasify-classified-listing' ),
			 'details'  => __( 'Details', 'clasify-classified-listing' ),
			 'amount'   => __( 'Amount', 'clasify-classified-listing' ),
			 'date'     => __( 'Date', 'clasify-classified-listing' ),
			 'customer' => __( 'Customer', 'clasify-classified-listing' ),
			 'status'   => __( 'Status', 'clasify-classified-listing' ),
			 'gateway'  => __( 'Gateway', 'clasify-classified-listing' ),
		 );
	...
	public function column_default( $payment, $column_name ) {
		switch ( $column_name ) {
			case 'amount':
				...
				$value  = CCP()->common->formatting->cl_currency_filter( CCP()->common->formatting->cl_format_amount( $amount ), cl_get_payment_currency_code( $payment->ID ) );
				break;
			case 'date':
			...
			case 'status':
				...
				break;
			case 'details':
				$value = '<a href="' . add_query_arg( 'id', $payment->ID, admin_url( 'edit.php?post_type=cl_cpt&page=cl-payment-history&view=view-order-details' ) ) . '">' . __( 'View Order Details', 'clasify-classified-listing' ) . '</a>';
				break;
			default:
	...
	public function column_email( $payment ) {
		...
		if ( cl_is_payment_complete( $payment->ID ) && ! empty( $email ) ) {
			$row_actions['email_links'] = '<a href="' . add_query_arg(
				...
				$this->base_url
			) . '">' . __( 'Resend Purchase Receipt', 'clasify-classified-listing' ) . '</a>';
		}
		...
		$row_actions['delete'] = '<a href="' . wp_nonce_url(
			...
			'cl_payment_nonce'
		) . '">' . __( 'Delete', 'clasify-classified-listing' ) . '</a>';

		$row_actions = apply_filters( 'cl_payment_row_actions', $row_actions, $payment );

		if ( empty( $email ) ) {
			$email = __( '(unknown)', 'clasify-classified-listing' );
		}
	...
	public function column_customer( $payment ) {
		...
		if ( ! empty( $customer_id ) ) {
			...
			$email = cl_get_payment_user_email( $payment->ID );
			$value = '<a href="' . esc_url( admin_url( "edit.php?post_type=listing&page=cl-payment-history&s=$email" ) ) . '">' . __( '(customer missing)', 'clasify-classified-listing' ) . '</a>';
		}
	...
	public function get_bulk_actions() {
		$actions = array(
			'delete'                 => __( 'Delete', 'clasify-classified-listing' ),
			'set-status-publish'     => __( 'Set To Completed', 'clasify-classified-listing' ),
			'set-status-pending'     => __( 'Set To Pending', 'clasify-classified-listing' ),
			'set-status-processing'  => __( 'Set To Processing', 'clasify-classified-listing' ),
			'set-status-refunded'    => __( 'Set To Refunded', 'clasify-classified-listing' ),
			'set-status-revoked'     => __( 'Set To Revoked', 'clasify-classified-listing' ),
			'set-status-failed'      => __( 'Set To Failed', 'clasify-classified-listing' ),
			'set-status-abandoned'   => __( 'Set To Abandoned', 'clasify-classified-listing' ),
			'set-status-preapproval' => __( 'Set To Preapproval', 'clasify-classified-listing' ),
			'set-status-cancelled'   => __( 'Set To Cancelled', 'clasify-classified-listing' ),
			'resend-receipt'         => __( 'Resend Email Receipts', 'clasify-classified-listing' ),
		);
