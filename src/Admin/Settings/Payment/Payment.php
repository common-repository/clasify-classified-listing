...
use Clasify\Classified\Admin\Settings\Payment\Paymentlist;
	...
	function cl_payment_history_page() {
		...
		if ( isset( $_GET['view'] ) && 'view-order-details' == $_GET['view'] ) {
			...
			if ( empty( $payment_exists ) ) {
				die( __( 'The specified ID does not belong to a payment. Please try again', 'clasify-classified-listing' ) );
			}
			$args                   = array();
			...
			cl_get_template( 'order_details.php', $args, '', CLASIFY_CLASSIFIED_PLUGIN_TEMPLATES_DIR . '/admin/' );
	...
	function cl_view_order_details_title( $admin_title, $title ) {
		...
		switch ( $_GET['cl-action'] ) :
			...
			case 'view-order-details':
				$title = __( 'View Order Details', 'clasify-classified-listing' ) . ' - ' . $admin_title;
				break;
			case 'edit-payment':
				$title = __( 'Edit Payment', 'clasify-classified-listing' ) . ' - ' . $admin_title;
				break;
			default:
				$title = $admin_title;
				break;
