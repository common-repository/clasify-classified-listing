...
class Setting_archive extends Archive {
					...
					<div class="listing-img-wrapper">
						...
						if ( isset( $archive_setting['sectionzero'] ) && count( $archive_setting['sectionzero'] ) > 0 ) {
							...
							foreach ( $archive_setting['sectionzero'] as $key => $section ) {
								...
								echo CCP()->admin->settings_instances->cl_listing_modal_container( $str );
								echo '</div>';
								if ( $co == 2 ) {
									echo '<div class="list-img-slide">
									<div class="click">
										<div><a href="#"><img src="' . CLASIFY_CLASSIFIED_PLUGIN_ASSETS . '/img/placeholder_light.png' . '" class="img-fluid mx-auto" alt="' . esc_attr__( 'Placeholder', 'clasify-classified-plugin' ) . '" /></a></div>
									</div>
								</div>';
								}
							}
	...
	function cl_archive_setting_view() {
		...
		$archive_setting = CCP()->admin->settings_instances->admin_settings_extend( $archive_setting, $this->cl_archive_default_setting() );
		ob_start();
		?>
